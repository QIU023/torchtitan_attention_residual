...
### Concrete gaps to close (vs the current `phase5_vlm_multimodal_sft/` code)
...
The C4-based alignment in `phase5_vlm_multimodal_sft/runs/align_pp4v2_*` validates that the
...
```bash
# rented box
INIT_CKPT=phase4_kimi_attnres_lm_pretrain/runs/.../checkpoint/step-8000 \
STEPS=20000 LOCAL_BS=8 GLOBAL_BS=32 SEQ_LEN=258 \
bash phase5_vlm_multimodal_sft/launch_train.sh   # FSDP=4, PP=1
```
...
`phase5_vlm_multimodal_sft/runs/align_pp4v2_naive_seed42` exists only as a sanity check
...
NGPU=4 PP=4 V=2 LOCAL_BS=1 GLOBAL_BS=12 STEPS=5000 \
    INIT_CKPT=path/to/phase4_kimi_attnres_lm_pretrain/weak_ckpt \
    bash phase5_vlm_multimodal_sft/launch_pp_adapter.sh

# Step 5: compare PP vs FSDP loss curves at matched steps
python phase5_vlm_multimodal_sft/compare_pp_vs_fsdp.py \
    --pp phase5_vlm_multimodal_sft/runs/arm2_pp4_adapter/tb \
    --fsdp phase5_vlm_multimodal_sft/runs/arm2_fsdp_baseline/tb
```

## Notes

* Code lives entirely in `phase5_vlm_multimodal_sft/` workspace dir. Not intended for
