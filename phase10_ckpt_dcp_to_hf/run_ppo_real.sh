...
CKPT="${CKPT:-$WS/phase5_vlm_multimodal_sft/runs/sft_v11_llava_instruct_150k_4d/checkpoint/step-490}"
if [[ ! -d "$CKPT" ]]; then
...
OUT=$WS/phase5_vlm_multimodal_sft/runs/ppo_real_torchtitan
...
torchrun \
    ...
    -m phase10_ckpt_dcp_to_hf.ppo_real_torchtitan \
    --ckpt "$CKPT" \
    --n-steps 50 \
    --seq-len 256 \
    --micro-bs 2 \
