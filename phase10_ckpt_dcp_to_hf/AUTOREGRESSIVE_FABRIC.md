...
* `phase10_ckpt_dcp_to_hf/inference_autoregressive.py` — generation loop with
  ``--mode {growing,single_token}`` switch.
* `phase10_ckpt_dcp_to_hf/run_autoregressive.sh` — runs both modes with tier_b
  NCCL trace + pipeline → ixia_config.json.
* `phase5_vlm_multimodal_sft/runs/inference_autoregressive_growing/tier_b_trace/` —
  growing-prefix mode trace artifacts.
* `phase5_vlm_multimodal_sft/runs/inference_autoregressive_single_token/run.log` —
