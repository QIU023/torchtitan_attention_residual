...
PY=$WS/phase10_ckpt_dcp_to_hf/two_phase_tp_smoke.py

run_mode() {
    local mode=$1
    local OUT=$WS/phase5_vlm_multimodal_sft/runs/two_phase_tp_${mode}
    ...
    cd "$WS"
    python phase7_nccl_traffic_catalog/extract_collectives.py "$TRACE" 2>&1 | tail -8
...
echo "  $WS/phase5_vlm_multimodal_sft/runs/two_phase_tp_allreduce/tier_b_trace/ixia_config.json"
echo "  $WS/phase5_vlm_multimodal_sft/runs/two_phase_tp_rs_ag/tier_b_trace/ixia_config.json"
echo "================================================================"
