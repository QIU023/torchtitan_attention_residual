...
run_workload() {
    ...
    local seq=$3
    local n=$4
    local OUT=$WS/phase5_vlm_multimodal_sft/runs/workload_${label}
    ...
    torchrun \
        ...
        -m phase10_ckpt_dcp_to_hf.inference_torchtitan \
        --ckpt "$CKPT" \
        --n-steps "$n" \
        --seq-len "$seq" \
        --micro-bs "$bs" \
    ...
    cd "$WS"
    python phase7_nccl_traffic_catalog/extract_collectives.py "$TRACE" 2>&1 | tail -5
    python phase7_nccl_traffic_catalog/expand_to_flows.py "$TRACE" 2>&1 | tail -3
...
for label in short_high_bs mid long prod; do
    f=$WS/phase5_vlm_multimodal_sft/runs/workload_${label}/tier_b_trace/ixia_config.json
    if [[ -f "$f" ]]; then
        sz=$(wc -c < "$f")
