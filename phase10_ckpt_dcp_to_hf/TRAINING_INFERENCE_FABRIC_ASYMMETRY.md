...
(see `phase7_nccl_traffic_catalog/expand_to_flows.py:_classify`). To split EP from PP cleanly, a
trainer-side commId-to-axis dump is required (future work, identified in
`phase7_nccl_traffic_catalog/FINAL_CATALOG.md`). For inference at PP=1 the entire `pp+ep`
