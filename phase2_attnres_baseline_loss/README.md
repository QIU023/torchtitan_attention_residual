...
The scripts in this folder assume `phase2_attnres_baseline_loss/` and `torchtitan/` are siblings:

```
<workspace>/                        # e.g. ~/work/ on the rental box
├── phase2_attnres_baseline_loss/                         # this folder: scripts + runbook + run outputs
│   ├── setup_env.sh
│   ├── launch.sh
│   ├── compare_losses.py
...
# 2. This phase2_attnres_baseline_loss/ folder (copy from your laptop, or keep in a gist and clone)
...
4. Runs the standalone smoke test (`phase2_attnres_baseline_loss/smoke_test_attn_res.py`).
...
bash run_train.sh --module llama3 --config llama3_debugmodel \
    --training.steps 100 --dump_folder ~/work/phase2_attnres_baseline_loss/runs/smoke_baseline

# AttnRes debug
bash run_train.sh --module attn_res --config debugmodel_attn_res \
    --training.steps 100 --dump_folder ~/work/phase2_attnres_baseline_loss/runs/smoke_attn_res
```
...
```bash
STEPS=2000 bash phase2_attnres_baseline_loss/launch.sh
```

### Override output directory

```bash
OUT_ROOT=~/experiments/run1 bash phase2_attnres_baseline_loss/launch.sh
```
...
```bash
conda activate attnres
tensorboard --logdir ~/work/phase2_attnres_baseline_loss/runs --port 6006 --bind_all
```
...
```bash
tail -f ~/work/phase2_attnres_baseline_loss/runs/baseline/train.log
```

### Check if the guardian flipped yet

```bash
ls -la ~/work/phase2_attnres_baseline_loss/runs/baseline/DONE 2>/dev/null && echo "baseline done" || echo "baseline still running"
```
...
```bash
python ~/work/phase2_attnres_baseline_loss/compare_losses.py \
    --baseline ~/work/phase2_attnres_baseline_loss/runs/baseline/tb \
    --attn_res ~/work/phase2_attnres_baseline_loss/runs/attn_res/tb \
    --out ~/work/phase2_attnres_baseline_loss/runs/comparison.png
```

Expected stdout:

```
[compare_losses] wrote ~/work/phase2_attnres_baseline_loss/runs/comparison.png
...
```bash
scp training-box:~/work/phase2_attnres_baseline_loss/runs/comparison.png .
scp training-box:~/work/phase2_attnres_baseline_loss/runs/baseline/train.log ./baseline.log
scp training-box:~/work/phase2_attnres_baseline_loss/runs/attn_res/train.log ./attn_res.log
```
...
Something's broken in the init. Re-run `phase2_attnres_baseline_loss/setup_env.sh` — it will
...
log line. Adjust the grep in `phase2_attnres_baseline_loss/launch.sh` (guardian window) to match.
...
```bash
grep 'Training completed' ~/work/phase2_attnres_baseline_loss/runs/baseline/train.log
```

If the line is there but `DONE` isn't, touch it manually:

```bash
touch ~/work/phase2_attnres_baseline_loss/runs/baseline/DONE
```
...
```bash
HF_REPO=NousResearch/Meta-Llama-3.1-8B bash phase2_attnres_baseline_loss/setup_env.sh
```
...
| File | Role |
| --- | --- |
| `phase2_attnres_baseline_loss/setup_env.sh` | Create conda env, install torch + torchtitan, download tokenizer, run tests |
| `phase2_attnres_baseline_loss/launch.sh` | Start tmux session with sequential baseline + attn_res runs |
| `phase2_attnres_baseline_loss/compare_losses.py` | Read TensorBoard events, plot loss curves, print final delta |
| `phase2_attnres_baseline_loss/smoke_test_attn_res.py` | Standalone torch-only smoke test (runs without torchtitan install) |
| `phase2_attnres_baseline_loss/README.md` | This runbook |
