...
The v8 orchestrator (`phase6/run_v8_crash_resilient_pretrain.sh`) auto-relaunched after every KDA Triton CUDA assert, exploiting the projector save/load fix from commit 57a4b47. **6 KDA crashes during the run, all recovered without projector reset.**
...
Smoke-tested `phase5/generate_caption.py` against v8/step-10000 with the trained projector loaded from the `mm_projector` ckpt entry:

```
$ torchrun --nproc_per_node=1 phase5/generate_caption.py \
    --ckpt phase5/runs/v8_.../checkpoint/step-10000 \
...
1. **`mm_projector` registered with checkpointer** (commit 57a4b47, `phase5/train_mm.py`). The multimodal trainer's projector + its AdamW state are now persisted via DCP. Any same-`dump_folder` auto-resume restores them. **Eliminates the ~50-100 step projector re-alignment penalty** that v1-v7 paid on every restart.
...
3. **Crash-resilient orchestrator** (commit fa1081d, `phase6/run_v8_crash_resilient_pretrain.sh`). Loop detects worker death, sleeps 30s, relaunches without `initial_load_path` so torchtitan auto-resumes from the latest in-dir ckpt. Each iter increments seed by 1.

## A5 redo (mid-save resume smoke) — PASS

After v8 finished, ran the re-fixed `phase6/run_a5_redo.sh`:
...
* `phase5/runs/v8_pretrain_resilient_from_v7_step800/checkpoint/step-10000` — final ckpt (loss 2.07, includes `mm_projector` entry)
* `phase5/runs/v8_pretrain_resilient_from_v7_step800/checkpoint/step-9800` — penultimate (also includes mm_projector)
* `phase5/runs/v8_pretrain_resilient_from_v7_step800/tb/` — full TB log across all 8 iters
* `phase5/runs/v7_pretrain_bs120_from_v6_step1200_BEST/checkpoint/step-800` — v8 init point (loss 2.79)
* `phase5/runs/arm1prime_fsdp_seed42_from_p4_8k/checkpoint/step-4000` — original GBS=32 caption story endpoint (loss 3.03)
