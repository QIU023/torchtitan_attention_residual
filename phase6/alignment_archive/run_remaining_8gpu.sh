...
SEED=42
ORCH_LOG="$WORKSPACE_DIR/phase6/orchestrator_8gpu.log"
...
run_config() {
    ...
    local out="$WORKSPACE_DIR/phase5/runs/8gpu_${cfg}_seed${SEED}"
...
bash "$WORKSPACE_DIR/phase6/run_alignment_reports.sh" >> "$ORCH_LOG" 2>&1 || true

# Also run extract_collectives on every tier_c trace
echo "" >> "$ORCH_LOG"
echo "[$(date)] extracting NCCL collectives from Tier C traces" >> "$ORCH_LOG"
for d in "$WORKSPACE_DIR/phase5/runs/8gpu_"*/tier_c_trace; do
    [[ -d "$d" ]] || continue
...
bash "$WORKSPACE_DIR/phase7/run_tier_b_a_traces.sh" >> "$ORCH_LOG" 2>&1 || true
...
for d in "$WORKSPACE_DIR/phase5/runs/8gpu_"*/tier_{b,a}_trace; do
    [[ -d "$d" ]] || continue
    /venv/main/bin/python "$WORKSPACE_DIR/phase7/extract_collectives.py" "$d" >> "$ORCH_LOG" 2>&1 || true
done
...
echo "[$(date)] To launch v10 multimodal pretrain: bash phase6/run_v10_pretrain.sh" >> "$ORCH_LOG"
