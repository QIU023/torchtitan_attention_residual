...
# under phase6/alignment_<config>.{txt,csv,png}.
...
ANCHOR_TB="$(_resolve_tb "$WORKSPACE_DIR/phase5/runs/8gpu_b0_fsdp8_seed42")"
if [[ -z "$ANCHOR_TB" || ! -d "$ANCHOR_TB" ]]; then
    echo "ERROR: B0 anchor TB not found under phase5/runs/8gpu_b0_fsdp8_seed42/tb/" >&2
    exit 1
fi
...
report() {
    local cfg="$1"
    local tb
    tb="$(_resolve_tb "$WORKSPACE_DIR/phase5/runs/8gpu_${cfg}_seed42")"
    ...
    /venv/main/bin/python phase5/compare_pp_vs_fsdp.py \
        --pp "$tb" \
        --fsdp "$ANCHOR_TB" \
        --noise-band 0.13 \
        --out-csv "$out_csv" \
