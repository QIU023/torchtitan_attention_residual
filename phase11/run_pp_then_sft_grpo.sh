...
cd "$WS"
LOG="$WS/phase11/run_pp_then_sft_grpo.log"
exec >>"$LOG" 2>&1
...
RUN_NAIVE=0 RUN_ADAPTER=1 bash "$WS/phase3/run_pp_pressure_test.sh"
echo "[$(date)] Stage A done"

# Find the latest pressure_test dir to reference results
PP_OUT=$(ls -dt "$WS"/phase3/runs/pressure_test_* 2>/dev/null | head -1)
if [[ -n "$PP_OUT" ]]; then
