...
TRACE_DIR=${TRACE_DIR:-$WS/phase11/rlhf/trace_grpo_kimi_attnres_${TS}}
mkdir -p "$TRACE_DIR"
...
CKPT_DCP=${CKPT_DCP:-$WS/phase5/runs/mm_sft_447m_full/checkpoint/step-3100}
CKPT_HF=${CKPT_HF:-$WS/phase5/runs/mm_sft_447m_full/hf_step3100}
...
timeout 2400 /usr/bin/python3 phase11/rlhf/run_grpo_llava_kimi.py \
...
if (( n_logs > 0 )); then
    echo "==> phase7/extract_collectives.py"
    /usr/bin/python3 "$WS/phase7/extract_collectives.py" "$TRACE_DIR/" \
        && echo "   collective_summary.csv: $(wc -l < $TRACE_DIR/collective_summary.csv) rows"

    echo "==> phase7/expand_to_flows.py"
    /usr/bin/python3 "$WS/phase7/expand_to_flows.py" "$TRACE_DIR/" --world-size 8 \
        && echo "   flows.csv: $(wc -l < $TRACE_DIR/flows.csv) rows"

    echo "==> phase7/flows_to_ixia.py"
    /usr/bin/python3 "$WS/phase7/flows_to_ixia.py" "$TRACE_DIR/" \
