...
Counterpart to ``phase10/dcp_to_hf_kimi_attn_res.py`` (LM-only).
Differences:

1. The DCP state dict contains a top-level ``mm_projector.projector.*``
   block written by ``phase5/multimodal_model.py``'s ``Projector`` —
...
class _Projector(nn.Module):
    """Mirror of phase5/multimodal_model.py:Projector. Used only to
