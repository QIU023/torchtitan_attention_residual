...
`phase3/` for the 175M Llama 200K-step naive vs adapter alignment
that already validated it; a Kimi-Linear flavored repeat of that
benchmark belongs in `phase4/experiments/kimi_pp_adapter/` (TBD).
...
- Loss curves overlaid with MA-50 smoothing (use the existing
  `phase3/plot_naive_vs_adapter.py` adapted for these two flavors)
