...
#
# Output:
#   phase11_rlhf_grpo_infra/trace_kimi_tp8_shard{0,1}/
#   phase11_rlhf_grpo_infra/trace_kimi_3d_shard{0,1}/
#   phase11_rlhf_grpo_infra/trace_qwen3_3d_shard1/
#   phase5_vlm_multimodal_sft/runs/ppo_smoke_no_vllm/tier_b_trace/    (re-run, in-place)
set -uo pipefail

WS=/root/torchtitan_attention_residual
KIMI_MODEL=$WS/phase11_rlhf_grpo_infra/hf/lm_base
QWEN3_MODEL=$WS/phase11_rlhf_grpo_infra/hf_qwen3_attn_res
...
postprocess() {
    ...
    if (( n_logs > 0 )); then
        python3 "$WS/phase7_nccl_traffic_catalog/extract_collectives.py" "$trace_dir/" >/dev/null 2>&1 \
            && echo "    extract OK ($(wc -l < $trace_dir/collective_summary.csv) rows)"
        python3 "$WS/phase7_nccl_traffic_catalog/expand_to_flows.py" "$trace_dir/" --world-size 8 >/dev/null 2>&1 \
            && echo "    flows OK ($(wc -l < $trace_dir/flows.csv) rows)"
        python3 "$WS/phase7_nccl_traffic_catalog/flows_to_ixia.py" "$trace_dir/" --world-size 8 >/dev/null 2>&1 \
            && echo "    ixia OK"
    else
...
run_inference_trace "$WS/phase11_rlhf_grpo_infra/trace_kimi_tp8_shard0" "$KIMI_MODEL" 8 1 1 0 "$KIMI_BACKEND"
run_inference_trace "$WS/phase11_rlhf_grpo_infra/trace_kimi_tp8_shard1" "$KIMI_MODEL" 8 1 1 1 "$KIMI_BACKEND"
run_inference_trace "$WS/phase11_rlhf_grpo_infra/trace_kimi_3d_shard0" "$KIMI_MODEL"  2 2 2 0 "$KIMI_BACKEND"
run_inference_trace "$WS/phase11_rlhf_grpo_infra/trace_kimi_3d_shard1" "$KIMI_MODEL"  2 2 2 1 "$KIMI_BACKEND"

# ---------- Qwen3 cross-carrier verification ----------
QWEN3_BACKEND="attention_backend='flashinfer',"
run_inference_trace "$WS/phase11_rlhf_grpo_infra/trace_qwen3_3d_shard1" "$QWEN3_MODEL" 2 2 2 1 "$QWEN3_BACKEND"

# ---------- Phase 9-B PPO toy (re-run, random-init MLP, no ckpt) ----------
echo ""
echo "==> phase 9-B PPO smoke (re-run for fresh trace)"
PPO_OUT=$WS/phase5_vlm_multimodal_sft/runs/ppo_smoke_no_vllm
...
torchrun --nproc_per_node=8 \
    ...
    -m phase9_post_training_ppo_trace.ppo_smoke_no_vllm \
    >"$PPO_OUT/run.log" 2>&1
rc=$?
echo "    boot+gen rc=$rc"
...
echo ""
echo "=== ALL DONE ==="
ls -la "$WS"/phase11_rlhf_grpo_infra/trace_*/ixia_config.json 2>/dev/null | awk '{print "  "$NF}'
