...
#   * No live phase5_vlm_multimodal_sft.train_mm processes
#
# When both conditions hold, fires post_sft_vlm_smoke.sh which runs
# DCP -> HF safetensors -> SGLang Engine boot smoke -> 10-sample
# qualitative eval. Output captured to phase11_rlhf_grpo_infra/auto_post_sft.log.
#
...
LOG="$WS/phase11_rlhf_grpo_infra/auto_post_sft.log"
ORCH_LOG="$WS/phase11_rlhf_grpo_infra/sft_447m_orchestrator.log"
...
while true; do
    ...
    last_step=$(grep -aoE "step:\s*[0-9]+" "$WS/phase5_vlm_multimodal_sft/runs/vlm_447m_sft_instruct/train.log" 2>/dev/null \
        | tail -1 | grep -oE "[0-9]+")
    ...
    bash "$WS/phase11_rlhf_grpo_infra/post_sft_vlm_smoke.sh"
    rc=$?
