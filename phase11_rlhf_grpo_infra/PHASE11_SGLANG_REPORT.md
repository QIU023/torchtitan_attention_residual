...
| File | Purpose |
| --- | --- |
| `phase11_rlhf_grpo_infra/dump_aligned_smoke.py` | 1.4B aligned ckpt builder |
| `phase11_rlhf_grpo_infra/dump_qwen3_attn_res_smoke.py` | Qwen3 ckpt builder |
| `phase11_rlhf_grpo_infra/bench_attn_res.py` | 4-mode bench harness (subprocess-isolated env) |
| `phase11_rlhf_grpo_infra/run_long_ctx_bench.sh` | TP×ctx sweep |
| `phase11_rlhf_grpo_infra/probe_cuda_graph.py` | cuda-graph compat smoke |
| `phase11_rlhf_grpo_infra/probe_memory.py` | per-rank GPU mem sampling via nvidia-smi |
| `phase11_rlhf_grpo_infra/run_all_traces.sh` | Full trace sweep (4 inference + 1 PPO) |
| `phase11_rlhf_grpo_infra/bench_results/*.json` | Bench raw output |
| `phase11_rlhf_grpo_infra/trace_*` | NCCL traces + collective_summary.csv + flows.csv + ixia_config.json |
...
| PPO production smoke | ✅ phase9_post_training_ppo_trace/ppo_actor_ref_real_ckpt.py — dual 1.4B model fwd/bwd + KL + clipped surrogate validated on real ckpt |
...
scope tracked in `phase9_post_training_ppo_trace/PPO_TRACE_DEFERRED.md`).
...
* Local-only env-compat patches in `torchtitan/distributed/*` and
  ...
  [`phase11_rlhf_grpo_infra/torchtitan_vast_ai_env_compat.patch`](torchtitan_vast_ai_env_compat.patch)
  with the rationale documented in
  [`phase11_rlhf_grpo_infra/TORCHTITAN_VAST_AI_PATCHES.md`](TORCHTITAN_VAST_AI_PATCHES.md).
...
* Pretrain (step-8000 ckpt) ready for resume:
  ...
  OUT_DIR=$PWD/phase4_kimi_attnres_lm_pretrain/runs/kimi_447m_aligned_block_attn_res_fsdp_paperhparams \
  NGPU=8 bash phase4_kimi_attnres_lm_pretrain/launch_from_scratch_paperhparams.sh
  ```
