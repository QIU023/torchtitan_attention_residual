...
SFT_DIR="$WS/phase5_vlm_multimodal_sft/runs/vlm_447m_sft_instruct/checkpoint"
...
HF_OUT="${HF_OUT:-$WS/phase11_rlhf_grpo_infra/hf/vlm_sft_${LATEST_STEP}}"
...
torchrun --nproc_per_node=1 --master-port=29521 \
    "$WS/phase11_rlhf_grpo_infra/dcp_to_hf_kimi_attn_res_vl.py" \
    --in "$SFT_CKPT" \
    --out "$HF_OUT" \
...
CUDA_VISIBLE_DEVICES=7 timeout 360 python3 "$WS/phase11_rlhf_grpo_infra/smoke_vlm_engine.py" \
    --model-path "$HF_OUT" \
    --tp-size 1

echo
echo "==> 3. Qualitative eval on 10 LLaVA-Pretrain images (TP=1 on GPU 7)"
CUDA_VISIBLE_DEVICES=7 timeout 600 python3 "$WS/phase11_rlhf_grpo_infra/eval_vlm_qualitative.py" \
    --model-path "$HF_OUT" \
    --num-images 10 \
