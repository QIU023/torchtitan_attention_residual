...
BENCH_OUT="$WORKSPACE/phase11_rlhf_grpo_infra/bench_results_prod_$TIMESTAMP"
mkdir -p "$BENCH_OUT"
...
KIMI_CKPT="$WORKSPACE/phase11_rlhf_grpo_infra/hf_kimi_48b_e8_dummy"
QWEN3_CKPT="$WORKSPACE/phase11_rlhf_grpo_infra/hf_qwen3_14b_attn_res_dummy"
...
if [[ ! -d "$KIMI_CKPT" ]]; then
    ...
    python3 "$WORKSPACE/phase11_rlhf_grpo_infra/dump_kimi_48b_attn_res_dummy.py" \
        --num-experts 8 --out "$KIMI_CKPT"
fi
if [[ ! -d "$QWEN3_CKPT" ]]; then
    echo "[2b] Dump Qwen3-14B + AttnRes (~14B params, ~28 GB safetensors)"
    python3 "$WORKSPACE/phase11_rlhf_grpo_infra/dump_qwen3_big_attn_res_dummy.py" \
...
python3 "$WORKSPACE/phase11_rlhf_grpo_infra/probe_memory.py" \
