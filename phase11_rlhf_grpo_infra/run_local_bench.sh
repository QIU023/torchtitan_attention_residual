...
BENCH_OUT="$WORKSPACE/phase11_rlhf_grpo_infra/bench_results_local_$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BENCH_OUT"
...
QWEN3_CKPT="$WORKSPACE/phase11_rlhf_grpo_infra/hf_qwen3_attn_res_dummy"
python3 "$WORKSPACE/phase11_rlhf_grpo_infra/dump_qwen3_attn_res_smoke.py" --out "$QWEN3_CKPT"

KIMI_CKPT="$WORKSPACE/phase11_rlhf_grpo_infra/hf_aligned_dummy"
if [[ "$SCOPE" != "quick" ]]; then
    echo "[3b] Kimi aligned dummy (~30s)"
    python3 "$WORKSPACE/phase11_rlhf_grpo_infra/dump_aligned_smoke.py" --out "$KIMI_CKPT"
fi

# 4. Bench
echo "[4] Bench Qwen3 TP=1 (~15 min, 3 modes)"
python3 "$WORKSPACE/phase11_rlhf_grpo_infra/bench_attn_res.py" \
    --model "$QWEN3_CKPT" \
    --tp 1 \
...
cd "$WORKSPACE"
rm -rf "$WORKSPACE/phase4_kimi_attnres_lm_pretrain/runs/lm_447m_fp8_paperalign_C/" 2>/dev/null
LOCAL_BS=4 GLOBAL_BS=384 LR=1.5e-3 WARMUP=150 STEPS=12750 \
    OUT_DIR="$WORKSPACE/phase4_kimi_attnres_lm_pretrain/runs/lm_447m_fp8_paperalign_C" \
    nohup bash phase4_kimi_attnres_lm_pretrain/launch_redo_paperalign_10B.sh \
    > "$WORKSPACE/phase4_kimi_attnres_lm_pretrain/fp8_paperalign_C.log" 2>&1 &
STAGE0_PID=$!
...
echo "Stage 0 pid=$STAGE0_PID, log: phase4_kimi_attnres_lm_pretrain/fp8_paperalign_C.log"
echo "==========================================="
