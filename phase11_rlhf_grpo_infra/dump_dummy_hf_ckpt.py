...
The shape mirrors what ``phase4_kimi_attnres_lm_pretrain/launch_paperhparams_break3.sh`` actually
...
``phase10_ckpt_dcp_to_hf/dcp_to_hf_kimi_attn_res.py`` and uploaded, swap the
...
Run::
    ...
    python3 phase11_rlhf_grpo_infra/dump_dummy_hf_ckpt.py --out phase11_rlhf_grpo_infra/hf_436m_random
"""
...
def _live_to_hf_state_dict(model, cfg) -> dict:
    ...
    Mirrors what ``phase10_ckpt_dcp_to_hf/dcp_to_hf_kimi_attn_res.py`` produces from a
...
def main():
    ...
    # phase10_ckpt_dcp_to_hf/dcp_to_hf_kimi_attn_res.py emits and what Moonshot's HF
