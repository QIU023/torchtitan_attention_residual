...
- The multimodal splice under SP: *vision_positions* index the global token axis, so the splice gathers the stream to Replicate, scatters the tower's features, and re-shards; the tower's plain output is wrapped as a replicated DTensor first, since the embedding's output is a DTensor and `copy_` refuses the mix.
