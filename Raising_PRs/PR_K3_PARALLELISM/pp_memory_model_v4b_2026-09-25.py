# 在 pp_memory_model_tight_2026-09-25.py 上加了 v4b 方案：V4 加上 81b30fd88（输入梯度 send 等到第一个能证明对端已收到的前向才释放），并在它之上逐项去掉剩余差距。
# 运行：PYTHONPATH=<pp_review5 的 checkout>:. python pp_memory_model_v4b_2026-09-25.py
...
def simulate(pp, vp, m, n_layers, block, scheme, act_per_layer=1.0, fix=(), movable_out=None):
    ...
    "per_block" (the store keeps each block from its arrival to the rank's last backward; with
    "tight" as well, only to the backward of the stage that brought it),
    ...
    # 81b30fd88: the input gradient of (s, mb) goes to s - 1 on the previous rank; the first
    # forward on this rank whose input that rank sent after consuming the gradient proves arrival
    grad_wait = {}
    for (s_, mb_), b_ in slot_b.items():
        if s_ == 0:
            continue
        consumed = slot_b.get((s_ - 1, mb_), end)
        r_ = s_ % pp
        waits = [
            slot_f[(q, m2)] for (q, m2) in slot_f
            if q % pp == r_ and q > 0 and slot_f[(q, m2)] > b_ and slot_f.get((q - 1, m2), end) > consumed
        ]
        grad_wait[(s_, mb_)] = min(waits) if waits else end
        ...
        for mb in range(m):
            ...
            if scheme == "v4" and "per_block" in fix:
                for blk in range(t.num_blocks):
                    ...
                    if bringers:
                        q0 = min(bringers)
                        stop = slot_b[(q0, mb)] + 1 if "tight" in fix else b_last + 1
                        add(r, slot_f[(q0, mb)], stop, 1)
            for s in stages_of[r]:
                ...
                if scheme == "base":
                    ...
                    if s > 0 and "grad_pin" not in fix:
                        add(r, b, grad_wait[(s, mb)] if early_grad else end, 1 + k_in[s])
                else:  # lower and tight
                    if s > 0:
                        add(r, f, b + 1, 1)
            ...
            if scheme == "tight":
                # each block only until the backward of the stage that brought it to this rank,
                # which is the last reader of that block on this rank in backward order
                for blk in range(t.num_blocks):
                    bringers = [
                        q for q in stages_of[r]
                        if blk in t.commits_at(q) or (q and blk in t.delta_to_send(q - 1))
                    ...
                    if bringers:
                        q0 = min(bringers)
                        add(r, slot_f[(q0, mb)], slot_b[(q0, mb)] + 1, 1)
            if scheme == "lower":
                for blk in range(t.num_blocks):
                    arrivals = [
...
def audit():
    cases = [
        ("5060 probe PP8 x VP2, 32 layers, block 4", 8, 2, 16, 32, 4, 3584 * 2048 * 2 / 2**30),
        ("H100 PP8 x VP4, 93 layers, block 12", 8, 4, 16, 93, 12, 8192 * 7168 * 2 / 2**30),
        ("H100 PP16 x VP2", 16, 2, 32, 93, 12, 8192 * 7168 * 2 / 2**30),
        ("GB300 PP4 x VP4", 4, 4, 16, 93, 12, 8192 * 7168 * 2 / 2**30),
        ("GB300 PP2 x VP8", 2, 8, 16, 93, 12, 8192 * 7168 * 2 / 2**30),
    ]
    print("| layout | V4 (09-24) | V4 + 81b30fd88 | then per_block | then open_copy | then hidden_out | per_block + open_copy | all three | report bound | tight |")
    print("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for label, pp, vp, m, nl, blk, unit in cases:
        def worst(sch, fix=()):
            per, dyn, _, _ = simulate(pp, vp, m, nl, blk, sch, 0.0, fix)
            return max(per[r] + max(dyn[r]) for r in range(pp)) * unit
        v4, v4b = worst("v4"), worst("v4b")
        pb, oc, ho = worst("v4b", ("per_block",)), worst("v4b", ("open_copy",)), worst("v4b", ("hidden_out",))
        pboc = worst("v4b", ("per_block", "open_copy"))
        allf = worst("v4b", ("per_block", "open_copy", "hidden_out"))
        print(f"| {label} | {v4:.1f} | {v4b:.1f} | {pb:.1f} | {oc:.1f} | {ho:.1f} | {pboc:.1f} | {allf:.1f} | {worst('lower'):.1f} | {worst('tight'):.1f} |")


def per_rank():
    """Every rank's block+hidden peak for each column of the audit table, with max and mean."""
    cases = [
        ("5060 probe PP8 x VP2, 32 layers, block 4, seq 3584, dim 2048", 8, 2, 16, 32, 4, 3584 * 2048 * 2 / 2**30),
        ("5060 PP8 x VP2, 46 layers, block 6, seq 2048, dim 2048", 8, 2, 16, 46, 6, 2048 * 2048 * 2 / 2**30),
        ("H100 PP8 x VP4, 93 layers, block 12", 8, 4, 16, 93, 12, 8192 * 7168 * 2 / 2**30),
        ("H100 PP16 x VP2", 16, 2, 32, 93, 12, 8192 * 7168 * 2 / 2**30),
        ("GB300 PP4 x VP4", 4, 4, 16, 93, 12, 8192 * 7168 * 2 / 2**30),
        ("GB300 PP2 x VP8", 2, 8, 16, 93, 12, 8192 * 7168 * 2 / 2**30),
    ...
    cols = [
        ("V4", "v4", ()),
        ("V4+81b", "v4b", ()),
        ("+per_block", "v4b", ("per_block",)),
        ("+open_copy", "v4b", ("open_copy",)),
        ("+hidden_out", "v4b", ("hidden_out",)),
        ("pb+oc", "v4b", ("per_block", "open_copy")),
        ("all3", "v4b", ("per_block", "open_copy", "hidden_out")),
        ("PR A", "v4b", ("per_block", "open_copy", "tight")),
        ("PR A+hidden", "v4b", ("per_block", "open_copy", "hidden_out", "tight")),
        ("report", "lower", ()),
        ("tight", "tight", ()),
    ]
    for label, pp, vp, m, nl, blk, unit in cases:
        peaks = {}
        for name, sch, fix in cols:
            per, dyn, _, _ = simulate(pp, vp, m, nl, blk, sch, 0.0, fix)
            peaks[name] = [(per[r] + max(dyn[r])) * unit for r in range(pp)]
        print(f"\n{label} (block+hidden, GiB, modelled)")
        print("| rank | " + " | ".join(n for n, _, _ in cols) + " |")
        print("|---:|" + "---:|" * len(cols))
        for r in range(pp):
            print(f"| {r} | " + " | ".join(f"{peaks[n][r]:.1f}" for n, _, _ in cols) + " |")
        print("| max | " + " | ".join(f"{max(peaks[n]):.1f}" for n, _, _ in cols) + " |")
        print("| mean | " + " | ".join(f"{sum(peaks[n]) / pp:.1f}" for n, _, _ in cols) + " |")
