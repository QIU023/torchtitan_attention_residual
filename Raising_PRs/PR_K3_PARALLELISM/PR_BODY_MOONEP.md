...
Notes for filing:
- 2026-09-22, the branch moves to MoonEP's public release (`33327eb`, 2026-09-20) in two commits over the six, "moonep: move onto the public release's interface" and "tests: follow the public release's contract". The release takes the local rows and the prefetch pool as separate tensors and fences inside its kernels, so the per-owner mapping loop, the private-context reach for barrier handles, the two group barriers, the `[E]` fp32 gradient table and the global offsets are all gone; 131 lines added against 205 removed. The slot count is fixed at E / R there, so `num_prefetch_slots` and the expert width that derived it are gone with it, and the `B = E / R - 1` and `B = 1` experiments cannot be run on the public release. Verified so far: the CPU files pass, and the interleaving probe on two H100s is bitwise between the sequential and interleaved orders. The on-device test and the h100 cell still need a run on the release, and the requirements section below still names the old pin.
