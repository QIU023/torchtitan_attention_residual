...
- Known open item, stated rather than hidden: across two very different forced load shapes the plan's `experts_to_copy` did not change, so the balancing benefit under skew is not yet realized end to end (hot cells: moonep 345 vs standard 336 tps). Whether the planner needs an explicit load input we are not passing is under investigation against the moonep source; the transport itself is correct per the gradient probe.
