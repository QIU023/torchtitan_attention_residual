...
To file: the fork branch `k3_pp_offload` exists (created 2026-09-17 from `pp_offload_review1`, head `d49bb388b`, one commit on 4312's head `de6f29514`); open the PR against `main` with base commits showing 4312's diff too (as 4380 and 4381 do), title as above, body from the paste section. The commit message still ends with a `(cherry picked from ...)` line, see the audit below.
