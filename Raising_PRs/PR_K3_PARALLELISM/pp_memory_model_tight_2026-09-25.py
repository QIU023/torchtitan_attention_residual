# 在 pp_memory_model_v2_2026-09-24.py 的基础上加了 tight 方案：block 只存到把它带到本 rank 的 stage 反向为止（该 block 在本 rank 上按反向顺序的最后一个读者）。
# 运行：PYTHONPATH=<pp_review5 的 checkout>:. python pp_memory_model_tight_2026-09-25.py
...
def audit():
    cases = [
        ("5060 probe PP8 x VP2, 32 layers, block 4, seq 3584 x 2048", 8, 2, 16, 32, 4, 3584 * 2048 * 2 / 2**30),
        ("H100 PP8 x VP4, 93 layers, block 12, 8K x 7168", 8, 4, 16, 93, 12, 8192 * 7168 * 2 / 2**30),
    ...
    for label, pp, vp, m, nl, blk, unit in cases:
        res = {sch: simulate(pp, vp, m, nl, blk, sch, 0.0) for sch in ("v4", "lower", "tight")}
        peaks = {sch: [res[sch][0][r] + max(res[sch][1][r]) for r in range(pp)] for sch in res}
        worst = {sch: max(v) for sch, v in peaks.items()}
        mean = {sch: sum(v) / len(v) for sch, v in peaks.items()}
        print(f"{label}, M={m}: block+hidden per rank, GiB (max / mean over {pp} ranks)")
        for sch in ("v4", "lower", "tight"):
            print(f"  {sch:6s} max {worst[sch] * unit:6.2f}  mean {mean[sch] * unit:6.2f}  per-rank " + " ".join(f"{x * unit:.1f}" for x in peaks[sch]))
        print(f"  tight vs lower: max -{(worst['lower'] - worst['tight']) * unit:.2f} GiB ({(1 - worst['tight'] / worst['lower']) * 100:.0f}%), mean -{(mean['lower'] - mean['tight']) * unit:.2f} GiB")
