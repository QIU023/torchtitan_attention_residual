...
PR 4312. `pp_review4` = `b1f82ba4a` (2026-09-13): `pipeline_parallel.py` and `configs.py` carry no change against main; the one-way split check sits in `Trainer.Config.__post_init__` (`trainer.py` +9); `k3_pp_text` = `ae2e7b9c4` needs the sync (approval) to carry it; `k3_pp_text` = `ae2e7b9c4` (fast-forward from `8de0c078c`, 2026-09-13). `8de0c078c` since 2026-09-13 (fast-forward from `dbc425403`: the split moved into Kimi K3, `pipeline_parallel.py` +4/-1 against main). Before that `dbc425403` since 2026-09-12 (fast-forward from `dd1c0b925`: the B200 cells, the per-rank stage count removed, the comment and docstring trims; GitHub: 31 commits, 16 files, +1324/-51, still `dirty` -- `torchtitan/config/configs.py` conflicts with main `56a721b64`, 17 commits past the base). Before that it was `dd1c0b925` (moved with lease from `75045fed5`, which was 19 commits on upstream/main `6e2ac3dcd`, to `66601a7fb`, then a test commit and the stage rebuild on top); that head is `pp_review4`: the same runtime minus the two transport commits, plus round 3, rebased onto main `d9ca9e55a` (23 commits). The transport port alone is `k3_pp_transport` = `8126172f8`, stacked on it. GitHub has reported the PR unmergeable since PR 4527 landed on 2026-09-09.
...
- Core: `pipeline_parallel.py` and `ParallelismConfig` are main's. `Trainer.Config.__post_init__` refuses an explicit `module_fqns_per_model_part` together with `pipeline_parallel_layers_per_stage` (in the trainer config rather than `ParallelismConfig`, whose `__post_init__` re-runs on every `dataclasses.replace`, including main's `pipeline_with_first_stage_modules`). Kimi K3's entry hands its own split to `pipeline_llm` through `module_fqns_per_model_part`.
...
### Changed files
    ...
    torchtitan/
      trainer.py                            +9/-0    an explicit split and pipeline_parallel_layers_per_stage are refused together
    torchtitan/models/kimi_k3/
      ...
      parallelize.py                        +196/-3  the pipelining entry: Kimi K3's own split, the stage rebuild, the tables, the transport switch; pipeline parallel off the unsupported list
    ...
    tests/unit_tests/cpu/
      test_kimi_k3_pp_layout.py             +207/-0  the tables: uneven split, cache on and off; Kimi K3's split, the pp8 x vp4 recipe's split, the shared debug model's depth (new)
      ...
      test_config_manager.py                +14/-0   the split-knob check, and a split written into ParallelismConfig later stays valid
