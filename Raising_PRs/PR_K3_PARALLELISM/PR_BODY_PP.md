...
- Core: none. `pipeline_parallel.py` and `ParallelismConfig` are main's; Kimi K3's entry hands its own split to `pipeline_llm` through `module_fqns_per_model_part`, which `pipeline_llm` takes in preference to `pipeline_parallel_layers_per_stage`.
