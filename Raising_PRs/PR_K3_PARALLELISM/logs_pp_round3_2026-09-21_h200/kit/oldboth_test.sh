#!/bin/bash
# The equalized reference on copies of BOTH first-lineage caches: the inductor cache of the first reference and the Triton cache of the dp1 cell (a copy of the first reference's, which the rerun deleted).
# equalization hack (a code change) from a second cold compile (a cache change). GPU 3, 100 steps.
set -u
TT=/workspace/tt_pp; OUT=/workspace/results/pp_r3; nm=dp1_ns_oldboth
...
( cd $TT && CUDA_VISIBLE_DEVICES=3 TORCHINDUCTOR_CACHE_DIR=$OUT/ind_$nm TRITON_CACHE_DIR=$OUT/tri_$nm torchrun --nproc_per_node=1 --master_port=47003 \
