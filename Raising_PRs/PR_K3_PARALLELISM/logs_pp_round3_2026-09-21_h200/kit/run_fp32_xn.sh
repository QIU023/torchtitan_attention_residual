#!/bin/bash
# fp32 whole stack cells with the total grad norm reduced in the reference's order (PP_NORM_EXACT), 100 steps, on the
# fp32 lineage cache, with the same dumps as the locate campaign. The order file comes from the locate reference's grads dump.
set -u
TT=/workspace/tt_pp; OUT=/workspace/results/pp_r3_locate; SRC=/workspace/results/pp_r3; cd $OUT
export VIRTUAL_ENV=/workspace/venv_k3 PATH=/workspace/venv_k3/bin:$PATH PYTHONPATH=$TT NCCL_NVLS_ENABLE=0
export GN_FP32=1 FP32_PROBE=1 GN_REPR=1 LOSS_REPR=1 PP_NORM_EXACT=$OUT/fqn_order.txt
python3 -c "import torch; print('\n'.join(torch.load('$OUT/grads_ref.rank0.pt').keys()))" > $OUT/fqn_order.txt
echo "fqn order: $(wc -l < $OUT/fqn_order.txt) names"
COMMON="-m torchtitan.train --module kimi_k3 --debug.seed 42 --debug.deterministic --metrics.log_freq 1 --training.steps ${STEPS:-100}"
...
cell() {  # cell <name> <gpus> <nproc> <port> <config> <flags...>
  ...
  echo "$nm rc=$rc steps=$(grep -a -c 'step: ' $OUT/$nm.log) grads=$(ls $OUT/grads_$nm.rank*.pt 2>/dev/null | wc -l) $(date -u +%T)"
}
cell vp2n_xn 0,1 2 48011 kimi_k3_debugmodel_c4_pp_naive $B4 $P2 $F32 &
cell vp2c_xn 2,3 2 48012 kimi_k3_debugmodel_c4 $B4 $P2 $F32 &
wait
cell pp4vp4n_xn 0,1,2,3 4 48013 kimi_k3_debugmodel_c4_16stages_naive $B4 $P4 $F32
python3 /workspace/kit/locate_compare.py $OUT ref ref2 vp2n vp2c pp4vp4n vp2n_xn vp2c_xn pp4vp4n_xn > $OUT/compare_xn.txt 2>&1
echo XN-DONE $(date -u +%T)
