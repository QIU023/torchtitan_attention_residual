...
- P7 (activation checkpointing off in both, flex pinned, matched accumulation): PENDING. Why this is the next suspect: last week's forward dumps counted 380 attention-residual calls per step on one GPU and 452 under the pipeline, so the two paths recompute different regions, and flex's backward reads what the recomputed forward saved.
