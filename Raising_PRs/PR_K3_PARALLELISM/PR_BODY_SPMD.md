...
- The multimodal splice under sequence parallel: the stream and the tokens are plain shards, and a placeholder run can cross the shard boundary, so `parallelize_kimi_k3` hands the model its tp group and `_splice_under_sequence_parallel` gathers the stream (`spmd.redistribute` `S(0) -> R`, reduce-scatter backward), splices on the whole sequence with the tokens, which sequence parallel leaves whole, and hands back the shard (`R -> S(0)`, all-gather backward); a one-rank gloo test checks it against the whole-sequence splice and the gradient routing.
