...
- Checkpoint resume on the new tree (dp1 and pp2, save at step 2, resume, re-run steps 3-4): the step-3 loss and grad norm are bitwise with the continuous run, the step-4 loss is not (`8.59245` vs `8.59860` at dp1). It is not the checkpoint: every parameter has Adam state (1002 of 1002; the 32 state-less tensors are the `expert_bias_E` buffers), and right before the step-3 optimizer step the weights, `exp_avg`, `exp_avg_sq`, `step`, the learning rate and every hyperparameter are bitwise identical between the continuous and the resumed process (fp64 checksums; two resumes are bitwise with each other; a constant learning rate changes nothing). What differs is the step-3 gradient itself, by `1e-3` relative on the first parameters, with the same weights and the same forward loss: the backward computes slightly differently in a process that starts at step 3 than in one that started at step 1. A runtime determinism question (kernel specialization on the process's allocation pattern is the leading candidate), not a state round-trip bug; tracked separately from your missing-state fix.
