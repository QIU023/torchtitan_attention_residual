...
- Core: `module_fqns_per_model_part` becomes `pipeline_parallel_module_fqns_per_model_part`, the prefix every other pipeline field carries.
  - `get_module_fqns_per_model_part` refuses a configured `pipeline_parallel_layers_per_stage` or split rather than write its own split over them; the copy of the config that reaches `pipeline_llm` carries that split and nothing else of the user's config changes. A different stage count goes in as an explicit split, as the B200 cell does for four stages per rank with core's `_generate_llm_fqn_per_model_part`.
