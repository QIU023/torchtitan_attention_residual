...
- **分支：** `k3_pp_text` = `pp_review5` = `4abc12940`，09-25 按用户要求同步。它是 `dbd13d397` 之上的一个 commit，由为 jinsooihm r4097059930 做的两处改动 squash 而成：
  ...
  - `TrainingEngine.Config` 拒绝 split 与 `layers_per_stage` 同时给定。
  - 旧 head 备份为 `backup/k3_pp_text_pre_20260925`。
  - 随后按用户要求再加一个 commit，删掉 `pipeline_parallel/PP_ATTN_RES_CACHE.md`。现在 `k3_pp_text` = `pp_review5` = `714267617`，PR 为 14 个 commit、21 个文件。下文粘贴段没有引用这个 md。
