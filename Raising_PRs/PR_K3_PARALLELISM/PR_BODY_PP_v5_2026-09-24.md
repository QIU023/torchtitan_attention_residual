...
Status (not for pasting): the text below describes `pp_review4` at `2141a4eeb`, sixteen commits on upstream main `9e159aed7`: the seven typed commits of round 4, tianyu's two applied suggestions (with every reference renamed; as applied on GitHub the PR head `9bb1b87f0` fails, see REBASE_DRAFTS_2026-09-23.md), and seven round 5 commits, one per review thread (tianyu 09-24: the draft figure, `vpp`, CP raises directly, the other two stage helpers private, `PPRankLocalCache` in `cache.py`, the `@overload`; shuhuayu 09-24: one B200 pipeline cell, pp2 with four stages per rank). NEEDS AN H100 RERUN BEFORE PASTING: the Results tables are still the ones measured at `ce67cece4` on main `7349a2282`, and the branch has since been rebased twice (main `b64103072`, then `9e159aed7`, whose #4704 refactors the TP projection backends in `kimi_k3/model.py`); the rule is to re-measure the body tables after every rebase. Only the row labels changed here (vp to vpp).
