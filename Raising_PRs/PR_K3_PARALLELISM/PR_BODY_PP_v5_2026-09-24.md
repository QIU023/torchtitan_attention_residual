...
- Core: `module_fqns_per_model_part` becomes `pipeline_parallel_module_fqns_per_model_part`, the prefix every other pipeline field carries.
  - `get_module_fqns_per_model_part` refuses a configured split rather than write its own over it; a configured `pipeline_parallel_layers_per_stage` sizes the split it derives, as main's helper does. The copy of the config that reaches `pipeline_llm` carries that split and nothing else of the user's config changes. A stage count the knob cannot express goes in as an explicit split, as the B200 cell does for four stages per rank with core's `_generate_llm_fqn_per_model_part`.
