...
- **分支：** `pp_review5` = `90ca18903`，已推到 fork，PR 分支暂不同步。按用户的要求从 `k3_pp_text` `dbd13d397` 切出，为 jinsooihm 的 r4097059930 cherry-pick 了两个 commit：
  - `901ef34de`：配置了 `layers_per_stage` 时，由它决定推导出的 split；
  - `90ca18903`（原 `d9b84e505`，去掉两行注释后 amend）：`TrainingEngine.Config` 拒绝 split 与 `layers_per_stage` 同时给定。
