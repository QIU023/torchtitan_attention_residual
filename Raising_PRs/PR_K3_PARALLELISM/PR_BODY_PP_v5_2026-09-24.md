...
- The split is core's. `get_module_fqns_per_model_part` pins the modules a model names to the ends of the auto split: `first_stage_module_fqns` to the first stage, as before, and now `last_stage_module_fqns` to the last; Kimi K3 pins the vision tower to the first stage, `output_res_proj` and `output_res_norm` to the last.
  - `pipeline_with_first_last_stage_modules` takes its split from that function and returns what `pipeline_llm` returns. Kimi K3 routes along the split, so it takes the split first and hands it to `pipeline_llm` itself. `pipeline_llm` and the trainer's contract are untouched, and the three upstream models that pin first-stage modules (`kimi_k2_7`, `muse_glimmer`, `qwen3_5`) reach the helper through `BaseModel.pipeline` and only follow the rename.
...
- Core: `module_fqns_per_model_part` becomes `pipeline_parallel_module_fqns_per_model_part`, the prefix every other pipeline field carries.
  - `get_module_fqns_per_model_part` refuses `layers_per_stage`, since the split it derives is sized by the schedule's default stage count; the copy of the config that reaches `pipeline_llm` carries that split and nothing else of the user's config changes. A different stage count goes in as an explicit split, as the B200 cell does for four stages per rank with core's `_generate_llm_fqn_per_model_part`.
