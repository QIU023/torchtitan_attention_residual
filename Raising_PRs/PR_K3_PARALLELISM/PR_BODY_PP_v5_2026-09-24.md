...
- Core: `module_fqns_per_model_part` becomes `pipeline_parallel_module_fqns_per_model_part`, the prefix every other pipeline field carries.
  ...
  - `ParallelismConfig.__post_init__` refuses a split together with `pipeline_parallel_layers_per_stage`, where `pipeline_llm` would silently drop the knob. The helper and Kimi K3's entry make the config copy that carries the derived split with `copy.copy`, which does not re-run that check, so the copy keeps the knob that sized the split, as on main, and nothing of the caller's config changes. A stage count the knob cannot express goes in as an explicit split, as the B200 cell does for four stages per rank with core's `_generate_llm_fqn_per_model_part`.
