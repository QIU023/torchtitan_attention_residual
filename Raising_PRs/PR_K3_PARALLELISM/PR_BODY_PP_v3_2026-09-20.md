...
- The shared `debugmodel` flavor is 17 layers, four runs of (3 KDA + 1 MLA) and a closing MLA as the 93 layer model has, in blocks of 4 (four full blocks and a partial one), vocabulary 2048 against the 2020 token test tokenizer, dim 256 with 4 heads, 8 experts with top 2, a 2 layer vision tower: 25.8 M parameters, from 1.4 B. Nine layers have every path the transport takes; 17 is the least depth in that pattern whose 19 units split into the 16 stages of pp4 x vp4, which the results below run. The CI cell is pp2 x vp2 on core's default split, [emb, 0-3] [4-8] [9-13] [14-16, head]: three of the five blocks open inside a stage, stage 2 receives two blocks and reads the first from its rank's store, stage 3 receives one and reads three. The pp2 x vp4 and pp4 x vp4 recipes in `torchtitan_recipes/tests/b200.py` are the vp4 rows of the results and not CI cells; no `layers_per_stage` reaches 8 or 16 stages for 19 units, so they spell their splits out through `pipeline_parallel_module_fqns_per_model_part`.
...
### Changed files
    ...
    torchtitan/models/kimi_k3/
      ...
      __init__.py                           +33/-12  registers the pipelining_fn; the debug model at 17 layers, dim 256, vocabulary 2048
    tests/unit_tests/cpu/
      ...
      test_kimi_k3_pp_layout.py             +210/-0  the tables: uneven split, cache on and off, the three recipes' splits (new)
    ...
    tests/integration_tests/b200.py         +6/-0    the pp2 x vp2 cell
    torchtitan_recipes/tests/b200.py        +64/-0   the pp2 x vp2 cell and the pp2 x vp4 / pp4 x vp4 recipes of the results, splits spelled out

### CI/CD Coverage

Five CPU test files run in the default suite: the exact block gradient test (the real `AttnResPipelineStage` under `ScheduleInterleaved1F1B` on four gloo ranks, integer valued block gradients so every sum is exact, bitwise against cache off and against a single device closed form over three steps in bf16 and fp32, with a forward only pass between steps), the layout tables (including the splits of the three pipeline recipes), the stage's carrier handling, the stage rebuild, and the core helper. One integration cell in the B200 suite, beside K3's existing cell, since Attention Gym's KDA runs only on SM100/SM103: pp2 x vp2 on two GPUs, where a rank receives blocks it already holds and three blocks open inside stages. A plain pp2 cell would exercise none of this: with one stage per rank the delta is the whole stack and the store is never read.
