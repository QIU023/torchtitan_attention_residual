...
- The multimodal inputs: main's `preprocess_inputs` declares layouts for the tokens only, so on main with the `spmd_types` default the multimodal debug flavor stops at its first step (`ValueError: spmd_types backend requires an SPMD layout for every tensor input ... ['grid_thw', 'pixel_values']`, also the path of the `kimi_k3_debugmodel_mm_fsdp2` cell); the override here gives `pixel_values` and `grid_thw` the DP-local, TP-invariant layout every VLM decoder shares.
