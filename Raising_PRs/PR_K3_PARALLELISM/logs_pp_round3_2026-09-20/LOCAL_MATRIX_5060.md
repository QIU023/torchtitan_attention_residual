...
Control without the quantile balancing hook (`noqb.sh`: `post_optimizer_build_fn = None`, the router keeps its initial expert biases; same seeds, same protocol, the reference's cache copied):

# noqb: 2048 tokens per step, dp2, no quantile-balancing hook
| cell | step 1 | step 10 | step 20 | step 50 | step 100 | step 1 | step 10 | step 20 | step 50 | step 100 (grad norm) | steps identical (loss, norm) | first loss difference |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
With the hook removed, dp2 x pp2 is identical to the matched dp2 reference on all 100 steps in the grad norm and on 97 in the logged loss (the three differ in the sixth digit with the norm, hence the weights, unchanged: the logged loss is a reduction over ranks and under PP that reduction includes the pipeline group). So the 2048 group's departure is the quantile balancing hook's behaviour under dp2 x pp2, not the block transport: the transport rows match the reference exactly once the hook is out, as in the 1024 group, where dp = 1 gives the hook no group to reduce over and the rows match with the hook in. What in the hook differs between dp2 and dp2 x pp2 is not established here; it is the first item for the H100 rerun and for the reply to tianyu, and it touches #4577 rather than this PR.

A print probe in the hook (`qb_probe_dp2_vs_pp2.log`: after the loss mesh reduction, per layer, the histogram sum, the rank's token count and the first four entries of the next expert bias, at the end of steps 1 and 2) shows the hook computing the same thing in both cells: the reduced histogram sums are 31952 then 30336 for every layer in both, and the eight per layer bias vectors at the end of step 1 and of step 2 are the same eight vectors, to six decimals, in dp2 and in dp2 x pp2 (split four and four over the two stages). So the departure is not in the hook's arithmetic as printed; it is somewhere between the identical bias and the step 2 loss, and it appears only with the hook installed. Left open; a step 1 weight dump after the optimizer step in both cells is the next probe, if the H100 rerun shows the same.
