...
Every cell: rc 0, 100 steps, the seed loaded on every rank (`local_matrix_cells.log`). Both references are warm reruns: the chain ran each reference on a fresh cache, every other cell on a copy of that finished cache, and the reference was then rerun on such a copy too, since on this box a cold compile and a warm one are not the same kernels (memory `gate-inductor-cache-confound`; the 1024 reference's first log had also been truncated by an accidental second launch, kept as `dp1_ns_overwritten_by_dup.log` in the scratchpad).
...
The 2048 group reproduces the 09-13 structure as well, once the reference is warm: `d2_pp2` and `d2_vp2n` are identical to the matched dp2 reference on all 100 steps, loss and grad norm, and `d2_vp2c`, `d2_dp2` (stock accumulation) and `d2_ep2` depart at step 2 in the same band (0.1% to 0.2% at step 10). The first reading of this group said the opposite, and the reason is worth keeping: the chain's dp2 reference was the one cell that ran on a fresh cache, and on this box the kernels a cold compile settles on are not the ones a process loading the finished cache uses, so `d2_dp2_ns_cold` sits 0.16% off `d2_dp2_ns` (warm) from step 2, with the grad norm 2.2% off at step 10, while every warm cell agreed with every other warm cell. Two things were tried before the cache was suspected and both were misread: removing the quantile balancing hook (`noqb.sh`) gave a cold reference and a warm pipeline cell that happened to stay within the sixth digit of each other, which looked like the hook mattered; a print and then a full precision dump of the hook (`qb_probe_dp2_vs_pp2.log`, `qbdump_compare.py`) showed its reduced histograms and next biases bitwise identical between dp2 and dp2 x pp2 at the end of steps 1, 2 and 3, which is true and is not where the difference was. The 1024 group did not show the problem only because its reference had to be rerun anyway, and the rerun was warm.

Protocol consequence for the H100 rerun and for `run_pp_local.sh`: the reference runs twice, once cold to fill the cache and once on a copy of its own finished cache, and the warm log is the reference; every cell, reference included, is then a warm run. The body's 09-13 tables were taken with one cache shared in place, reference first, which is the same thing only if in place reuse is bitwise on that box; it was on the H100 (dp2 x pp2 identical on all 100 steps) and is not here.
