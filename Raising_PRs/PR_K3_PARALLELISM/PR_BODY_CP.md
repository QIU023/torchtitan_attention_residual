...
- Entry: the projections run on the local sequence chunk, giving *q* `[L, H, Q]`, *kv* `[L, H, N+V]` and the headless rotary key *k_rope* `[L, R]`. The layer branches to Ulysses when a CP group was set on it (`model.py:131-135`; `apply_cp_kimi_k3` sets it, `parallelize.py:117-133`).
