...
### Changed files
    ...
    torchtitan_recipes/
      ...
      tests/b200.py                         +2/-2    the multimodal cell on the multimodal flavor
...
- The text-only variant question on the vision splice (tianyu-l): done the muse_glimmer way, `debugmodel` / `Kimi-K3` without the tower and `debugmodel_mm` / `Kimi-K3_mm` with it, so the text case never needs a dependency on an unused tower; what stays is the CP-shard case on the multimodal flavor (a shard whose positions hold no image tokens still has to issue the tower's collectives), inlined in the splice.
