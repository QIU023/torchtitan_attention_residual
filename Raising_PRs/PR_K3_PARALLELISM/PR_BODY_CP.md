...
- The boundary and the model: `set_gqa_inner_attention_local_map` (the stack's identity boundary) on the MLA inner attention; the head count is derived from the projection width so TP and CP compose without a branch; the splice's mesh lookup hands the model its cp group for the vision splice, whose plain-tensor branch rejects sequence parallel like the DTensor branch.
