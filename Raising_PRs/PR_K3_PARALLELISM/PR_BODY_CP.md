...
PR 4313. Branch `cp_review5` on the fork (`88f232df2`): stacked on the declarations PR (`spmd_decl_review1` `482f21753`, on upstream/main `390e2985b` with 4446 merged), then two commits. The first is the open upstream CP stack (PR 4322 / 4449 / 4450 at `860d5aa64d`) copied to unblock running, every copied file and class marked "copied from upstream open PR 4322/4449/4450 to unblock running; pending rebase and reconcile", the whole commit to be dropped at the rebase once the stack merges. The second (`88f232df2`) is this PR: the CP layer, re-stacked on 2026-09-05 with one conflict in `parallelize.py` against 4446 (the CP hook sits after the model assertion, before 4446's replicated annotation and mesh resolution). The declarations the CP layer needs are the base PR's now; the previous head of this branch, `223e97a23`, carried its own copy of them, which is what the PR branch `k3_cp_text` holds until the sync is approved.
