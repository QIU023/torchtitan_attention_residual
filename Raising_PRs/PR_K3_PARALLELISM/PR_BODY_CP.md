...
- The boundary and the model: `set_gqa_inner_attention_local_map` (the stack's identity boundary) on the MLA inner attention; the head count is derived from the projection width so TP and CP compose without a branch; the vision splice takes its cp group from the active SPMD mesh, as the kernels do; its plain-tensor branch rejects sequence parallel like the DTensor branch.
...
Rows within a table read the same samples (the loader shards documents by dp rank, so the three tables are three data streams and do not compare with each other). The cells ran in separate invocations with their own compile caches, so step 1 is the comparison and step 3 / 10 carry this flavor's own floor: the dp1 cell on another cache reads 12.52977 / 7.27107 / 2.98077 against 12.52977 / 7.36833 / 2.91045 here, with bitwise step-1 gradients (bf16 end to end at lr 8e-4 with a 2-step warm-up). The CP cells start 1e-2 above their dp reference at step 1, cp8 another 1e-2 (reproduced on both of its runs and on the generic Ulysses kernel, so the offset belongs to the CP degree, not to the packing). The four cp2 kernels compute the same forward (the step-1 loss) and differ only in how their backwards add the shared rope slice's gradient: per-parameter step-1 gradient norms agree to 1e-4 (median; 90th percentile 1e-3), elementwise the gradients differ by 1e-2 relative (median) with 0.1-0.2% of the elements changing sign, and the two upstream kernels sit exactly as far from each other (generic Ulysses against generic all-gather KV: 0.14% sign flips, 7e-3 median) as the packed ones from them; backward hooks on the MLA kernels locate the whole difference at one point: at the first MLA layer the backward meets, the arriving gradient, d q, d v and d k's nope columns are bitwise and only the shared rope slice's gradient is delivered in another order (on one head by the packed kernel, on every head by the generic one; 1 to 4 bf16 ulp apart once the model's expand sums the heads), which the backward of the layers below amplifies to 1.7e-2 by layer 3, the mechanism of the pipeline census. Located layer by layer on the first micro-batch: the first KDA layer's output under KCP differs from the single-sequence kernel's on bitwise-identical inputs at bf16 accumulation level (relative 3e-3, largest element 2.4e-4, on the rank holding the sequence start as on the other), and the random-init router flips on it (5% of the stream by layer 3, 60% at the head); the four MLA kernels are bitwise against each other. The same-data EP pair of the declarations PR shows the same amplification (EP on flips 1.4% of the gradient elements' signs at step 1).
...
### Changed files
    ...
    torchtitan/models/kimi_k3/
      ...
      model.py                              +285/-26  the declaration calls and local regions; the KDA-kernel check; the vision splice under CP, its cp group from the SPMD mesh
      parallelize.py                        +1/-2     context parallel off the unsupported list
