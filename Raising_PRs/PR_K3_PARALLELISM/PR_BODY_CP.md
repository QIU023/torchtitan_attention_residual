...
PR 4313. Branch `cp_review4` on the fork (`7ed39389b`). It sits on `cp_base_stack` (`a8a6f331f`), a scratch commit that is fegin's CP stack (PR 4322 / 4449 / 4450 at `860d5aa64d`) applied onto upstream/main `9b5f60c40`; above it the TP/SP commits (`tp_review2`), the spmd declarations (`spmd_review2`), the CP content of `cp_review3` (`a4322344d`) and the commit that moves it onto the stack (`7ed39389b`). The PR branch is synced only on the user's approval, and only once the stack has landed (the scratch commit is never filed). Paste between the markers; the header of the PR should say it stacks on the TP/SP and declaration commits and on fegin's CP stack.
