...
Status (not for pasting): the text below describes `pp_review4` at `3f9201ea0`, seven typed commits on upstream main `7349a2282` (re-homed 2026-09-22 after #4810 removed `ModelSpec` and the per-model `parallelize.py`; round 4 of 2026-09-23 addressed shuhuayu's 18 inline comments and the review body). Results are re-measured on 4 x H100 at `ce67cece4`, the head before the last two changes, which alter only the form of the loop-style guard and the type of one error (`logs_pp_round4_2026-09-23_h100/`, TABLES_PENDING until the run lands). `k3_pp_text` was moved to this head on 2026-09-23 08:20 UTC on the user's word (the previous head `4b47d1222` is pinned as `backup/k3_pp_text_pre_20260923`); the PR shows seven commits and 21 files. Open question for the user: after #4810 nothing in a config reaches `attn_res_cache=False`; the naive rows were run through a local switch on `KimiK3Model.pipeline`, and a field on `KimiK3Model.Config` (as the offload draft has) would give recipes the A/B. The two SVGs and the uploaded figure links are unchanged.
