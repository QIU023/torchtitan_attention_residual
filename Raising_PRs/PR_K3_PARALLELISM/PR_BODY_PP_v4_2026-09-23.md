...
Status (not for pasting): the text below describes `pp_review4` at `3f9201ea0`, seven typed commits on upstream main `7349a2282` (re-homed 2026-09-22 after #4810 removed `ModelSpec` and the per-model `parallelize.py`; round 4 of 2026-09-23 addressed shuhuayu's 18 inline comments and the review body). Results are re-measured on 4 x H100 at `ce67cece4`, the head before the last two changes, which alter only the form of the loop-style guard and the type of one error (`logs_pp_round4_2026-09-23_h100/`, tables A and B in; table C pending). `k3_pp_text` was moved to this head on 2026-09-23 08:20 UTC on the user's word (the previous head `4b47d1222` is pinned as `backup/k3_pp_text_pre_20260923`); the PR shows seven commits and 21 files. Open question for the user: after #4810 nothing in a config reaches `attn_res_cache=False`; the naive rows were run through a local switch on `KimiK3Model.pipeline`, and a field on `KimiK3Model.Config` (as the offload draft has) would give recipes the A/B. The two SVGs and the uploaded figure links are unchanged.
...
| cell | loss, step 1 | step 10 | step 50 | step 100 | grad norm, step 1 | step 10 | step 50 | step 100 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| fp32 dp1 [1] | `8.003350` | `3.484610` | `2.582840` | `2.505890` | `2.2419` | `1.3957` | `0.6767` | `0.8747` |
| fp32 dp1, stock accumulation [4] | `8.003350`<br>identical | `3.484610`<br>identical | `2.582840`<br>identical | `2.505890`<br>identical | `2.2419`<br>identical | `1.3957`<br>identical | `0.6767`<br>identical | `0.8747`<br>identical |
| fp32 pp2 x vp2, naive [2] | `8.003350`<br>identical | `3.484610`<br>identical | `2.583100`<br>+0.01% | `2.504910`<br>-0.04% | `2.2419`<br>identical | `1.3957`<br>identical | `0.6768`<br>+0.01% | `0.8723`<br>-0.27% |
| fp32 pp2 x vp2, cached | `8.003350`<br>identical | `3.484610`<br>identical | `2.583320`<br>+0.02% | `2.505170`<br>-0.03% | `2.2419`<br>identical | `1.3957`<br>identical | `0.6769`<br>+0.03% | `0.8739`<br>-0.09% |
| fp32 pp4 x vp4, naive [2][3] | `8.003350`<br>identical | `3.484610`<br>identical | `2.583870`<br>+0.04% | `2.505230`<br>-0.03% | `2.2419`<br>identical | `1.3957`<br>identical | `0.6771`<br>+0.06% | `0.8723`<br>-0.27% |
| fp32 pp4 x vp4, cached [3] | `8.003350`<br>identical | `3.484610`<br>identical | `2.582950`<br>+0.00% | `2.506550`<br>+0.03% | `2.2419`<br>identical | `1.3957`<br>identical | `0.6769`<br>+0.03% | `0.8749`<br>+0.02% |

- [1] to [4] as above; the stock accumulation row is identical to the reference on all 100 steps, loss and grad norm; the two naive rows are identical through step 34, pp2 x vp2 cached through step 36 and pp4 x vp4 cached through step 41 in the loss, and through steps 35, 39, 35 and 43 in the printed grad norm
- the two whole stack rows leave the reference at step 35 here, at step 15 on the H200 run of the previous head. Located there, at `42ad4f3c5`: at step 1 every parameter gradient (459/459), every parameter after the update (481/481) and every micro-batch loss are bitwise with the reference; from step 2 the total grad norm differs in its last one to three fp32 units on most steps. This rerun repeats the cells, not the dumps
...
- with the per parameter norms gathered over the pp ranks and reduced in the reference's order (`PP_NORM_EXACT`, a probe switch, measured on the H200 at `42ad4f3c5`) the two naive rows are identical to the reference on all 100 steps, loss and grad norm, and their total norm bitwise on every step; the cached row under the same switch still leaves, the cache's own sum order (212 of its 459 step-1 gradients differ from the reference at the rounding level). The mechanism is core's and the rebase does not touch it, so the control was not rerun
