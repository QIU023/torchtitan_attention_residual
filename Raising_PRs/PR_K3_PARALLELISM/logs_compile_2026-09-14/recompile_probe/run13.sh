...
run(){ local tree=$1 gpu=$2 name=$3 cache=$4 layers=$5 tiny=$6 steps=$7
  ( source /workspace/venv_bfx9/bin/activate && cd $S/$tree && CUDA_VISIBLE_DEVICES=$gpu TRITON_CACHE_DIR=$cache/triton TORCHINDUCTOR_CACHE_DIR=$cache/inductor TORCHINDUCTOR_COMPILE_THREADS=8 TORCH_LOGS=recompiles \
    ...
    -m torchtitan.train --module kimi_k3 --config kimi_k3_debugmodel $B --training.steps $steps $C --dump-folder $S/out_$name > $S/$name.log 2>&1 ); echo "$name rc=$?" >> $S/status13.txt; rm -rf $S/out_$name; }
( run rp_base 0 rp_base_tiny24 $S/cache_rpb 24 1 2; run rp_base 0 rp_base_tiny33 $S/cache_rpb 33 1 2; run rp_base 0 rp_base_tiny93 $S/cache_rpb 93 1 2 ) &
( run rp_mark 1 rp_mark_tiny24 $S/cache_rpm 24 1 2; run rp_mark 1 rp_mark_tiny33 $S/cache_rpm 33 1 2; run rp_mark 1 rp_mark_tiny93 $S/cache_rpm 93 1 2; run rp_mark 1 rp_mark_full24 $S/cache_w 24 0 3 ) &
wait; echo ALL-DONE >> $S/status13.txt
