...
run(){ local tree=$1 gpu=$2 name=$3 cache=$4
  ( source /workspace/venv_bfx9/bin/activate && cd $S/$tree && CUDA_VISIBLE_DEVICES=$gpu TRITON_CACHE_DIR=$cache/triton TORCHINDUCTOR_CACHE_DIR=$cache/inductor TORCHINDUCTOR_COMPILE_THREADS=8 TORCH_LOGS=recompiles \
    ...
    -m torchtitan.train --module kimi_k3 --config kimi_k3_debugmodel $B --training.steps 3 $C --dump-folder $S/out_$name > $S/$name.log 2>&1 ); echo "$name rc=$?" >> $S/status16.txt; rm -rf $S/out_$name; }
run eb24 0 eb_compiled24 $S/cache_w &
run eb33 1 eb_compiled33 $S/cache_33 &
wait; echo ALL-DONE >> $S/status16.txt
