...
Re-run on the same 4 x H100, now on `c4_test` as text-only rows: at 1024 tokens per step the reference memorised the 32-sample debug set within the 100 steps, so its late steps measured memorisation. Step 1 is bitwise in every cell; at step 100 every pipeline cell is within 0.22% of the reference loss at 1024 tokens (order-only noise floor 0.19%) and within 2.1% at 2048 tokens with dp2 (floor 0.24%, dp2 x ep2 0.93%). The cached rows move further than the naive ones because the rank cache changes the order in which a block's gradient contributions are summed in bf16 (the deposits of the stages that read it from the cache first, then the gradient coming down the chain); every deposit is counted against the layout, and in a separate float32 probe (8 x 5060 Ti, pp4 x vp4, step-1 gradients) the cache-on vs cache-off difference falls from 7e-2 to 9e-6 relative while cache off stays bitwise with dp1.
