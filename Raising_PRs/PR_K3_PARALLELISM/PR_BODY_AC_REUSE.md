...
For PR 4656, repurposed (user, 2026-09-15, option a): retitle 4656 to the line above, replace its body with the paste section, and sync `k3_ac_reuse_attention` to review branch `ac_review3` = `ea1316606` (two commits on main `0ae22167c`: the recompute `d1c829f06` = the old `fd0a9b6c6` cherry-picked clean, and its CPU test). The region declarations (`615908fcd`, MLA/KDA `torch_remat` regions for RegionAC) are out: the report's paragraph (sec 5.2.2) says nothing of the kind, and reading "AC reuse" as reusing attention activations under AC is what put 4656's current body off course. Verdict table against the paragraph: `phase13_k3like_48b_posttrain/REBASE_CPMM_DEP_AC_2026-09-15.md` section 1 (the block stack's per-block re-cat, sentence 1 of the paragraph, is not in this PR either; a separate decision).

Numbers: the table below was measured on the pre-rebase head (main `b21f7d43e`) on one RTX 5060 Ti; the recompute commit is unchanged by the rebase, but per the H100 rule the body's table is rerun on H100 on `ea1316606` before filing, and the 5060 only smokes (`int0915_logs/run_ac4656.sh`: dp1 with AC none / selective / full, seeded, 3 steps). The RegionAC row of the old table is gone with the regions. Test plan names the new test file. Not run on this head: the 33-layer and pipeline cells.
