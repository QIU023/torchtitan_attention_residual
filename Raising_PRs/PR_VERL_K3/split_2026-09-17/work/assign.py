...
VARIANTS = {
    IMPL: {
        ...
        1299: [("02", "        gen = _gen()\n"
                      ...
                      "        return gen, None\n"),
               ("03", None)],
        1576: [("03", '    """``{fqn: wrapper}`` for every LoRA-wrapped linear in the module.\n'),
               ("05", '    """``{fqn: wrapper}`` for every KimiLoRALinear in the module.\n')],
        1580: [("03", "    qualified suffixes, and it skips subtrees it cannot wrap). A config-derived\n"),
               ("05", "    qualified suffixes, and it skips the KDA subtree structurally). A config-derived\n")],
        1713: [("03", "    path is the 4-D ``A_log`` reshape, which ``apply_lora`` never reaches\n"),
               ("05", "    path is the 4-D ``A_log`` reshape, and ``apply_lora`` skips the KDA subtree\n")],
        1731: [("03", "        # every layer, and a graft-only target can have no HF\n"),
               ("05", "        # every layer, and a graft-only target (the K3 attention gate) can have no HF\n")],
        1954: [("01", "                input_ids, position_ids, sp_size=pad_multiple, pad_value=pad_id\n"
                      "            )\n")],
        1990: [("04", "            # from the positions, the shards and the layouts. Hand it\n"),
               ("05", "            # from the positions, the shards, the KDA routing and the layouts. Hand it\n")],
        2014: [("01", "        if self._folded_token_stream:\n"),
               ("04", "        elif self._folded_token_stream:\n")],
        2015: [("01", "            # A model whose forward takes the packed stream's document offsets (a\n"),
               ("05", "            # A model whose forward takes the packed stream's document offsets (KDA's\n")],
        2016: [("01", "            # linear-attention recurrence, which would otherwise run across the\n"),
               ("05", "            # recurrence and short convolution, which would otherwise run across the\n")],
        1955: [("04", None)],
        2025: [("01", "                # them by consumer (a flex BlockMask, varlen offsets, and so on).\n"),
...
MOVES = {
    IMPL: [
        ...
        ((1947, 1954), "after", 1955),    # TP padding goes after the CP call's closing paren
        ((1956, 1958), "after", 2029),    # old multimodal update stays at the end of the block
    ],
    "verl/workers/config/engine.py": [
