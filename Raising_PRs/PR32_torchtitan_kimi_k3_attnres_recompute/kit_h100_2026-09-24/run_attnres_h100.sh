#!/bin/bash
# PR 4780 on the H100: upstream main against attnres_review1, AC off and selective AC, plus the fla override.
# Usage: MAIN=<upstream main checkout> HEAD=<attnres_review1 checkout> VENV=<venv with torch, torch_remat, attn-gym> \
#        FLA=<dir holding fla 0.6.0 from git main> OUT=<dir> FLAVOR=attnres_debug|attnres_wide \
#        TOKENS="512 4096" STEPS=20 GPUS="0 1 2 3" bash run_attnres_h100.sh
# One GPU per cell (dp1). For each token count every cell runs on its own copy of ONE warm cache
# (inductor and Triton), filled by main with AC off and one fused step; main_none_rerun is the
# noise floor on that lineage. The KDA capability guard is lifted for SM90 in both checkouts
# (undo with `git -C <tree> checkout -- torchtitan/models/kimi_k3/kda.py`).
set -uo pipefail
HERE=$(cd "$(dirname "$0")" && pwd)
: "${MAIN:?}" "${HEAD:?}" "${VENV:?}" "${FLA:?}" "${OUT:?}"
...
cell() {  # cell <name> <tree> <gpu> <cache> <steps> <tokens> <ac...>
  ...
  local extra=""
  case "$name" in *fused*) extra="--override.imports torchtitan.overrides.fused_attnres.fused_attnres" ;; esac
  ( cd "$tree" && PYTHONPATH="$FLA:$HERE:." CUDA_VISIBLE_DEVICES=$gpu \
    ...
    "$VENV/bin/torchrun" --nproc_per_node=1 --rdzv_backend c10d --rdzv_endpoint=localhost:$((29700 + gpu)) \
      ...
      --dump-folder "$OUT/$name/dump" $extra "$@" > "$OUT/$name/train.log" 2>&1
    echo "rc=$?" >> "$OUT/$name/train.log" )
}

for tokens in $TOKENS; do
  W="$OUT/cache_t${tokens}_warm"
  rm -rf "$W"; mkdir -p "$W" "$OUT/t${tokens}_warm_main" "$OUT/t${tokens}_warm_fused"
  echo "=== t$tokens: warming the shared cache"
  cell "t${tokens}_warm_main" "$MAIN" "${GPUS[0]}" "$W" "$STEPS" "$tokens" activation-checkpoint:none
  cell "t${tokens}_warm_fused" "$HEAD" "${GPUS[0]}" "$W" 1 "$tokens" activation-checkpoint:none
  specs=(
    "main_none $MAIN none" "main_none_rerun $MAIN none" "head_none $HEAD none" "head_fused_none $HEAD none"
    "main_sel $MAIN selective" "head_sel $HEAD selective" "head_fused_sel $HEAD selective"
  )
  i=0
  ...
  "$VENV/bin/python" "$HERE/tables_attnres.py" "$OUT" \
    "t${tokens}_main_none:t${tokens}_main_none_rerun,t${tokens}_head_none,t${tokens}_head_fused_none" \
    "t${tokens}_main_sel:t${tokens}_head_sel,t${tokens}_head_fused_sel" | tee "$OUT/table_t${tokens}.md"
done
