...
cases = {
    "uniform bf16": [torch.randn(64, dtype=torch.bfloat16) for _ in range(8)],
    "uniform fp32": [torch.randn(64) for _ in range(8)],
    # A few params kept in fp32 while the rest train in bf16 -- ordinary mixed precision.
    "mixed bf16+fp32": (
        [torch.randn(64, dtype=torch.bfloat16) for _ in range(6)]
