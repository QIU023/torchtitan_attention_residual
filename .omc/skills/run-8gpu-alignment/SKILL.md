...
## Purpose
Drive `phase6_upstream_pr_prep/run_8gpu_alignment_matrix.sh` end-to-end on the local 8-GPU box
...
in `phase6_upstream_pr_prep/README.md` — do not attempt them unless the user says the upstream
fix landed).
...
2. **Run the matrix**
   - `bash phase6_upstream_pr_prep/run_8gpu_alignment_matrix.sh 2>&1 | tee phase6_upstream_pr_prep/orchestrator_8gpu.log`
...
3. **Parse per-variant reports**
   - For each variant `V` in `{a2_fsdp2_pp4, a3_fsdp2_pp2_tp2, all4d_fsdp2_pp2_tp2_ep2, noppc_fsdp4_tp2_ep2}`:
     - Read `phase6_upstream_pr_prep/alignment_8gpu_${V}.txt`.
     - Extract the final-step loss delta vs the recorded baseline.
     - Mark PASS if `|Δ| ≤ 0.13` (the nondeterminism band from phase 3),
       FAIL otherwise.

4. **Write the summary**
   - Append a short table to `phase6_upstream_pr_prep/SESSION_8GPU_summary.md` under a new
     dated heading.
...
- Do not edit `phase6_upstream_pr_prep/run_8gpu_alignment_matrix.sh` from this skill — that
