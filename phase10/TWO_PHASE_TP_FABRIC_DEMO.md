...
### AllReduce baseline (`phase5/runs/two_phase_tp_allreduce/tier_b_trace/`)
...
### RS+AG two-phase (`phase5/runs/two_phase_tp_rs_ag/tier_b_trace/`)
...
`phase7/FINAL_CATALOG.md` future-work item 5.

## Files

* `phase10/two_phase_tp_smoke.py` — both modes in one file (mode
  controlled by `--mode {allreduce,rs_ag}`)
* `phase10/run_two_phase_smoke.sh` — runs both back-to-back, captures
  separate tier_b traces, runs the standard pipeline → 2 ixia_config
  files for side-by-side IXIA loading.
* `phase5/runs/two_phase_tp_{allreduce,rs_ag}/tier_b_trace/` —
