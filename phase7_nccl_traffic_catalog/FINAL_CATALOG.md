...
|---|---|---|---|---|
| **v11 4D pretrain** | FSDP=2 × PP=2 × TP=2 × EP=2 (V=2) | `phase5_vlm_multimodal_sft/runs/v11_4d_*/tier_b_trace/` | ✓ | 5000 step run, 13 attempts, 18 GB raw → 210 MB CSV.gz |
| **v12 4D pretrain (no TP)** | FSDP=2 × dp_rep=2 × PP=2 × EP=2 (V=2) | `phase5_vlm_multimodal_sft/runs/v12_trace_50step/tier_b_trace/` | ✓ | 50 step trace from step-5000 ckpt (production trace lost in retry-loop cleanup; this is the post-hoc replacement) |
| **SFT (post-train)** | same as v11 (FSDP=2×PP=2×TP=2×EP=2) | `phase5_vlm_multimodal_sft/runs/sft_v11_llava_instruct_150k_4d/tier_b_trace/` | ✓ | 490 step on LLaVA-Instruct-150K |
| 8gpu_a2 (alignment) | FSDP=2 × PP=4, V=2 | `phase5_vlm_multimodal_sft/runs/8gpu_a2_*/tier_*_trace/` | ✓ (×3 tiers) | older, GBS=16 |
| 8gpu_a3 (alignment) | FSDP=2 × PP=2 × TP=2, V=2 | `phase5_vlm_multimodal_sft/runs/8gpu_a3_*/tier_c_trace/` | ✓ | older, GBS=16 |
| 8gpu_b0 (alignment) | FSDP=8 (no PP), V=1 | `phase5_vlm_multimodal_sft/runs/8gpu_b0_*/tier_*_trace/` | ✓ (×3 tiers) | older, GBS=16 (DP-only) |
| **5D MODE=B (llama3 + CP)** | PP=2 × FSDP=2 × CP=2 (3 fabric axes) | `phase5_vlm_multimodal_sft/runs/5d_mode_b_llama3_pp_fsdp_cp/tier_b_trace/` | ✓ | 50 step llama3_debugmodel; **adds CP coverage** (nranks=4 AllGather/ReduceScatter from CP×FSDP group). DSv3+CP path crashes with mixed Tensor/DTensor in scaled_dot_product_attention; llama3 dense path is stable |
| **PPO smoke (vLLM-free)** | actor sub-mesh (ranks 0-3) + ref sub-mesh (ranks 4-7) + cross-mesh world_pg KL | `phase5_vlm_multimodal_sft/runs/ppo_smoke_no_vllm/tier_b_trace/` | ✓ | 50 step toy MLP smoke; **adds cross-mesh KL exchange signature** — nranks=8 Broadcast (KL scalars 800 ops, <1 KB) + nranks=4 AllReduce 16-256 MB (sub-mesh grad sync, 800 ops); see `phase9_post_training_ppo_trace/ppo_smoke_no_vllm.py` |
...
Documented in `phase7_nccl_traffic_catalog/expand_to_flows.py:_classify`.
...
```python
import json
cfg = json.load(open('phase5_vlm_multimodal_sft/runs/v11_4d_*/tier_b_trace/ixia_config.json'))
...
in `phase7_nccl_traffic_catalog/flows_to_ixia.py` for standard Ethernet.
...
1. **commId-aware axis dump**: add a hook in torchtitan trainer that
   ...
   `phase7_nccl_traffic_catalog/extract_collectives.py` to read commId from log lines and
   join with `phase7_nccl_traffic_catalog/comm_axis_map.csv` for accurate axis labels.
...
3. ~~**Real PPO trace**~~ ✓ done in Phase 10 Stage F: real
   ...
   `phase5_vlm_multimodal_sft/runs/ppo_real_torchtitan/tier_b_trace/ixia_config.json`
   and `phase10_ckpt_dcp_to_hf/PHASE10_FABRIC_REPORT.md`.
...
5. **Block AttnRes two-phase computation** (paper-aligned production
   ...
   `phase10_ckpt_dcp_to_hf/PHASE10_FABRIC_REPORT.md`; impl pending.

## Phase 10 additions to the catalog (inference + RLHF)

| Run / config | mesh | trace path | ixia_config | notes |
|---|---|---|---|---|
| **Stage D inference** | FSDP=4 x TP=2 x EP=2 (PP=1) | `phase5_vlm_multimodal_sft/runs/inference_torchtitan_phase4_step8000/tier_b_trace/` | ✓ | 50 fwd-only steps, phase4 step-8000 ckpt; **0 ReduceScatter** distinguishes from training |
| **Stage F real PPO** | FSDP=4 x TP=2 x EP=2 | `phase5_vlm_multimodal_sft/runs/ppo_real_torchtitan/tier_b_trace/` | ✓ | 50-step real RLHF with kimi_linear AttnRes actor + frozen ref + KL + PPO ratio loss; ~3x FSDP volume vs Stage D inference (2 fwd + 1 bwd) |

See `phase10_ckpt_dcp_to_hf/PHASE10_FABRIC_REPORT.md` for the cross-regime
