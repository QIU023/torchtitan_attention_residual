...
#
# Usage:
#   bash phase7/run_5d_fabric_trace.sh           # mode A (default)
#   MODE=B bash phase7/run_5d_fabric_trace.sh    # DSv3 5D
#   MODE=C bash phase7/run_5d_fabric_trace.sh    # LLaMA-3 + TP
set -u

MODE="${MODE:-A}"
WORKSPACE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT_DIR="$WORKSPACE_DIR/phase7/traces/5d_fabric_${MODE}"
...
mkdir -p "$OUT_DIR"
LOG="$WORKSPACE_DIR/phase7/orchestrator_5d_${MODE}.log"
exec >>"$LOG" 2>&1

# Disk pre-flight (see phase6/DISK_DISCIPLINE.md). 50-step tier_b trace
...
# Run via torchtitan's standard train.py (not phase5/train_mm — this
...
python "$WORKSPACE_DIR/phase7/extract_collectives.py" "$TRACE_DIR" || true
python "$WORKSPACE_DIR/phase7/expand_to_flows.py" "$TRACE_DIR" || true
python "$WORKSPACE_DIR/phase7/flows_to_ixia.py" "$TRACE_DIR" --mode aggregated || true
