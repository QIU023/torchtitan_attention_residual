#!/usr/bin/env bash
# Watcher: auto-publish phase7/archive at chain orchestrator milestones.
#
# Tails phase6/a3_v10_3d_orchestrator.log, fires publish_archive.sh
...
publish_once() {
    ...
    if grep -q "$marker" "$ORCH_LOG" 2>/dev/null; then
        ...
        bash "$WORKSPACE_DIR/phase7/publish_archive.sh" "$tag"
        touch "$STATE/$marker"
    fi
}
