...
#
# Usage:
#   bash phase7/publish_archive.sh ["commit message tag"]
#
# Default commit subject: "phase7/archive: refresh trace tarballs"
...
SIZE=$(du -sh phase7/archive | cut -f1)

git commit -m "$(cat <<EOF
phase7/archive: refresh trace tarballs ($TAG)

Idempotent re-pack via phase7/pack_traces.sh. Updated $CHANGED files,
total archive size now $SIZE. SNAPSHOT entries replaced with full
post-completion tarballs as in-flight runs finish.

Generator: phase7/publish_archive.sh
EOF
)" 2>&1 | tail -3 || {
