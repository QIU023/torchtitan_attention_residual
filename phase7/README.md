...
Record into `phase7/traces/<config_id>/tier_<X>/`:

```bash
NCCL_DEBUG=INFO \
NCCL_DEBUG_SUBSYS=COLL,INIT \
NCCL_DEBUG_FILE=phase7/traces/<config_id>/tier_<X>/nccl-rank-%h-%p.log \
TORCH_NCCL_TRACE_BUFFER_SIZE=20000 \
TORCH_NCCL_DUMP_ON_TIMEOUT=1 \
TORCH_NCCL_USE_COMM_NONBLOCKING=1 \
nsys profile \
  --trace=cuda,nvtx,nccl,osrt \
  --output=phase7/traces/<config_id>/tier_<X>/nsys-%h-%p \
...
```
phase7/traces/<config_id>/tier_<X>/
...
A per-config `phase7/traces/<config_id>/INDEX.md` lists all tiers
present and their relative realism rank, so a user opening that
directory immediately sees which trace to replay first.

Post-processing (`phase7/extract_collectives.py`, to be written) parses
...
| Tooling | Status |
|---|---|
| `phase7/nccl_trace_capture.sh` (env wrapper) | pending |
| `phase7/extract_collectives.py` (NCCL log → CSV) | pending |
| `phase7/configs/*.sh` (one per config × tier launcher) | pending |
| `phase7/pattern_catalog.md` (final consolidation, replay-priority table) | pending |
