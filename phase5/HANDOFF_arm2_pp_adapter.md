...
TORCHTITAN_ATTNRES_CACHE=1 NGPU=4 STEPS=50 \
    bash phase3/launch_pp4_naive_4gpu.sh
# (or the closest existing launcher; check phase3/runs/ for last
...
* Pull the **old** Phase 4 step-12500 ckpt (val ~3.73, NOT the new
  retrain). Stored at `phase4/runs/kimi_436m_block_attn_res_fsdp_overnight/checkpoint/step-12500/`
...
`phase5/multimodal_dataset.py:collate_with_pad` already pads to
...
NGPU=4 PP=1 LOCAL_BS=3 GLOBAL_BS=12 STEPS=2000 INIT=fresh \
    bash phase5/launch_train.sh   # Arm 1 launcher with same data
```
...
* **Phase 3 handoff**: `phase3/handoff_status_20260421.md` —
  per-rank cache distribution + the math behind P-1 delta blocks
* **Phase 4 retrain context**: `phase4/README.md`, especially the
  "Continuation-pretrain to 100K steps" section
* **Cache adapter design doc**: `phase3/adapter_design.md`
  (state machine + invariants; the local-cache-capture grad-bridge
  story is here)
* **Existing phase5 code**: `phase5/multimodal_*.py` + `train_mm.py`
