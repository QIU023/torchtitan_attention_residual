...
Source `phase6_upstream_pr_prep/preflight_disk.sh` at launcher start; it implements this
pattern check.

### Rule 3 — Mandatory pre-launch `df` check

`phase6_upstream_pr_prep/preflight_disk.sh` MUST run before any torchrun and refuse to
...
Any run estimated > 1 hour MUST start `phase6_upstream_pr_prep/disk_watchdog.sh` as a
...
`phase6_upstream_pr_prep/preflight_disk.sh` ALSO does:
...
Any multi-stage orchestrator (e.g. `phase11_rlhf_grpo_infra/run_phase6_closure_then_pretrain.sh`)
...
- [ ] sources `phase6_upstream_pr_prep/preflight_disk.sh`
- [ ] starts `phase6_upstream_pr_prep/disk_watchdog.sh` if estimated runtime > 1h
...
3. Re-run `phase7_nccl_traffic_catalog/auto_publish_watcher.sh` to clean any stale sweep ckpts
