...
```
run completes → phase7_nccl_traffic_catalog/pack_traces.sh → phase7_nccl_traffic_catalog/publish_archive.sh
              → automatic git commit + push to GitHub
              → delete the run's checkpoint dir if it ever wrote one
```

`phase7_nccl_traffic_catalog/auto_publish_watcher.sh` already does the publish part; add
post-publish cleanup to it (rm any `checkpoint/` under
`phase5_vlm_multimodal_sft/runs/8gpu_*` whose recipe says it's an alignment or trace run).

### 4. Pre-launch disk check

Add to `phase6_upstream_pr_prep/launch_8gpu_mm.sh`: at script start, if
...
`phase6_upstream_pr_prep/launch_8gpu_mm.sh` env knobs to make the policy automatic:
...
1. `rm -rf phase5_vlm_multimodal_sft/runs/8gpu_*/checkpoint/` — frees ~120 GB
2. Patch `phase6_upstream_pr_prep/launch_8gpu_mm.sh` per §6
3. Update `phase6_upstream_pr_prep/run_v10_pretrain.sh` to set `CHECKPOINT_ENABLED=1
   SAVE_FREQ=500 KEEP_K=2` (its current behavior)
4. Update `phase6_upstream_pr_prep/run_a3_then_v10_3d.sh` (if it ever re-runs alignment)
   to set `CHECKPOINT_ENABLED=0`
5. Update `phase7_nccl_traffic_catalog/auto_publish_watcher.sh` to nuke any
