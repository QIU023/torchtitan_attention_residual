...
- `phase4_kimi_attnres_lm_pretrain/runs/.../step-8000` ckpt scp'd from 4-GPU box (15 GB).
...
| File | Role |
|---|---|
| `phase6_upstream_pr_prep/launch_8gpu_mm.sh` | Generic mesh + recipe + tier driver. Required env: `OUT_DIR FSDP PP TP CP EP STEPS LOCAL_BS GLOBAL_BS`. Optional: `V ADAPTER FLAVOR STUDENT_CKPT SEED DETERMINISTIC COMPILE TRACE_TIER TRACE_STEPS`. Dumps `recipe.json` per run + opens NCCL trace if `TRACE_TIER` set. Used by everything below. |
| `phase6_upstream_pr_prep/run_remaining_8gpu.sh` | Phase 6 alignment matrix orchestrator — runs B0 → A2 → A3 → A6 → P7C4 → P7C5, then chains alignment reports → trace extraction → tier B → tier A → catalog gen. Writes to `phase6_upstream_pr_prep/orchestrator_8gpu.log`. |
| `phase6_upstream_pr_prep/run_v10_pretrain.sh` | 5000-step continued multimodal pretrain (GBS=120 LBS=15 FSDP=8) from step-8000. Optional; user starts manually after alignment matrix. |
| `phase6_upstream_pr_prep/run_alignment_reports.sh` | Wraps `phase5_vlm_multimodal_sft/compare_pp_vs_fsdp.py` for each config; emits `phase6_upstream_pr_prep/alignment_8gpu_<cfg>.{txt,csv,png}`. Resolves the timestamped TB subdir automatically. |
| `phase6_upstream_pr_prep/check_8gpu_status.sh` | One-shot status snapshot (running procs, GPU state, last-step + ERROR markers per run, orchestrator tail). |
| `phase7_nccl_traffic_catalog/extract_collectives.py` | Parses `NCCL_DEBUG=INFO COLL` logs into a structured CSV (`opname, count, dtype, bytes, size_bucket, nranks, root`) per trace dir. Verified on B0: 232k+ rows, FSDP all-gather/reduce-scatter dominate. |
| `phase7_nccl_traffic_catalog/run_tier_b_a_traces.sh` | Tier B (GBS=120, 50 steps) + Tier A (GBS=384, 100 steps) trace recording across 6 configs. ~12 h wallclock total. |
| `phase7_nccl_traffic_catalog/build_pattern_catalog.py` | Walks all `phase5_vlm_multimodal_sft/runs/8gpu_*/tier_{a,b,c}_trace/collective_summary.csv`, emits `phase7_nccl_traffic_catalog/pattern_catalog.md` with replay-priority table + per-config tier histograms + cross-config Tier A comparison. |
...
7. **Phase 7 trace catalog**: `phase7_nccl_traffic_catalog/extract_collectives.py` (NCCL log → CSV), `phase7_nccl_traffic_catalog/build_pattern_catalog.py` (consolidator), `phase7_nccl_traffic_catalog/pattern_catalog.md` (replay-priority ranked).
...
- v10 multimodal pretrain (script ready at `phase6_upstream_pr_prep/run_v10_pretrain.sh`,
...
3. `phase7_nccl_traffic_catalog/pattern_catalog.md` as documentation of which collectives
