...
`phase5_vlm_multimodal_sft/train_mm.py` (workspace location; lands at
...
* Resolves the image-sentinel token id from a per-tokenizer registry
  (`phase5_vlm_multimodal_sft/sentinel_registry.py`) with startup collision check, instead
...
Full ablation report: `phase6_upstream_pr_prep/cache_adapter_ablation.md`. The closed-form
...
phase5_vlm_multimodal_sft/tests/`:
...
* `kimi_linear/tests/test_multimodal_model.py` — multimodal scatter
  forward shape
* `phase5_vlm_multimodal_sft/tests/test_pp_vision_plumbing.py` (4 tests) — vision_embeds +
...
* `phase5_vlm_multimodal_sft/tests/test_variable_image_count.py` (7 tests, this PR) —
...
* `phase5_vlm_multimodal_sft/tests/test_sentinel_registry.py` (9 tests, this PR) — registry
...
pretrain (`phase6_upstream_pr_prep/run_v8_crash_resilient_pretrain.sh`): on KDA Triton
...
| 5 | Multimodal trainer (LLaVA-Pretrain) — FSDP arm + PP+adapter arm with synthetic C4 | `phase5_vlm_multimodal_sft/{train_mm,multimodal_dataset,multimodal_model}.py` |
