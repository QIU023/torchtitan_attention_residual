...
| `spmd_review2` | `00f97b417` | `tp_review2` + the two spmd_types declaration commits |
| `cp_review3` | `b40f8f4ff` | `spmd_review2` + the CP content of `cp_review2` as one commit (Ulysses / all-gather KV / KCP kernels owning their collectives, the SP guard, the cp2 flavors on spmd_types) |
...
| `cp_review4` | `609827058` | `cp_base_stack` + TP + spmd + the CP content of `cp_review3` + the move onto the stack (packed MLA kernels on 4450's interface, the KDA transform in the recipe, every copy from 4322 deleted) |
