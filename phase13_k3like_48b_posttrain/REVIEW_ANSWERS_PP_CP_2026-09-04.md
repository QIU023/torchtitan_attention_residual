...
**Added: `_install_step_drop_patch`, `on_microbatch_end`, the VP drop-guard, 2026-04-20.** The cache is shared by a rank's virtual stages, so a micro-batch's entries cannot be dropped when the *first* virtual stage finishes its backward. The adapter marks micro-batches seen at `backward_one_chunk` exit and drops everything after `pp_schedule.step()` returns, from the rank's last virtual stage only. This is the second wrapper. It also means we do **not** yet do what the paper describes as "released as soon as the micro-batch finishes": entries live until step end, so cache memory scales with the number of in-flight micro-batches (the 04-21 log's envelope: `|rank_cache_at_entry| * B * T * D * 2 * M`). The per-micro-batch release is a known gap; the `pp_review1` integration branch attacks the memory from a different side (host offload of own-rank commits, and peer offload through the Mooncake transfer engine).
...
| `_install_step_drop_patch` (wraps `pp_schedule.step`) | evicts the rank cache at step end | no schedule lifecycle hook for "all micro-batches done" | a schedule callback at micro-batch and step end; with it, per-micro-batch release becomes possible |
...
| `model.py:236` cat at the start of the layer, then `_apply_attention_residual` with `prefix_sum` None | mathematically identical (the value matrix stacks the same blocks in the same order), so it is a readability refactor that must come with a dp1 bitwise check. We will do it on the review branch after the numerics runs above are recorded. |
...
| `__init__.py:460/461/513/514` debugmodel defaults, irregular shape, block size 12, deducible `full_attention_layers` | agreed on all four once the even-split gate is gone (the only reason the 32-layer, block-16 flavor exists is that 24 and 12 do not tile the pp x vp products). Plan: one `_debugmodel(attn_backend, *, num_layers)` with `full_attention_layers` derived as `(3 KDA + 1 MLA) * k + remainder` in the same rule as the 93-layer model, `attn_res_block_size` fixed at 12, and a deliberately non-divisible layer count so the uneven split is what CI exercises. This reruns the matrix. |
...
| `pipeline_adapter.py:140` only-incremental transfer, and release as soon as the micro-batch finishes | incremental transfer: yes, the delta is exactly `accumulated - receiver_cache` and the wire carries nothing else (per-hop sizes in the 04-21 table). Release per micro-batch: no, entries are dropped at step end (2.4). This is a real gap against the report and the honest answer is that per-micro-batch release needs a schedule lifecycle hook (3.2, item 3) or another wrapper. |
