#!/bin/bash
# cp2 x pp2 and cp2 x tp2 (tower cut at threshold 96) on the tree with the CP refusal lifted, through mx4 with the
# matrix's seed checkpoint and a copy of its warm inductor cache. Run after the matrix is DONE.
set -uo pipefail
...
d=$(ls -td /workspace/mx4_int0916_*/ | head -1)
...
cp2_ag_cut|2|cpmm_probe|kimi_k3_mm_allgather_kv_cp2_min96|" $M/mx4.sh int0916_cpx
