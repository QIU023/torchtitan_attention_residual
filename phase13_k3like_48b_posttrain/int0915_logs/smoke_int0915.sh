#!/bin/bash
# 3-step GPU smokes of the 09-15 integration tree (no seed checkpoint: rc and loss only).
set -uo pipefail
TITAN=/tmp/wt_int0915
OUT=/workspace/.smoke_int0915/$(date +%m%d_%H%M%S); mkdir -p $OUT
