#!/bin/bash
# Seeded matrix on the rebased tree 45ed0e3c1 (k3_int_20260916c): the morning's 16 cells plus the five CP-composition
...
cd /workspace && TITAN=/tmp/wt_int0916 VENV=/workspace/venv_bfx9 PYPRE=$S:/workspace/pylib/attn_gym_main CFG=kimi_k3_debugmodel \
...
pp2|2|$RB|kimi_k3_debugmodel_pp2_vp2|$D1 --parallelism.num_pp_microbatches 16
fsdp2_pp2|4|$RB|kimi_k3_debugmodel_pp2_vp2|$D2 --parallelism.num_pp_microbatches 8
tp2_pp2|4|$RB|kimi_k3_debugmodel_pp2_vp2|$D1 $TP2 --parallelism.num_pp_microbatches 16
fsdp2_tp2_pp2|8|$RB|kimi_k3_debugmodel_pp2_vp2|$D2 $TP2 --parallelism.num_pp_microbatches 8
fsdp2_pp2_ep2|4|$RB|kimi_k3_debugmodel_pp2_vp2|$D2 $EP2 --parallelism.num_pp_microbatches 8
...
tp2_cp2_pp2|8|cpmm_probe|kimi_k3_mm_allgather_kv_cp2_pp2_min96|$TP2 --parallelism.no-enable-sequence-parallel --parallelism.num_pp_microbatches 16" $M/mx4.sh int0916c
echo "MATRIX DONE"
