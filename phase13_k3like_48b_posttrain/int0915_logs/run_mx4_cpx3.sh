#!/bin/bash
# cp2 x tp2 (tower cut at threshold 96), both flavours, on the tree with the CP refusal lifted and the sub-CP gather in a
# local SPMD region; mx4 with the matrix's seed checkpoint and a copy of its warm inductor cache.
set -uo pipefail
...
cp2_ul_cut|2|cpmm_probe|kimi_k3_mm_ulysses_cp2_min96|" $M/mx4.sh int0916_cpx3
