#!/bin/bash
# Dynamic CP on PR 4639, seeded: the 192-patch debug image cut over the pair (threshold 96) against the tower replicated.
# the two mm CP recipes with the large image partitioned (default) and with the tower forced replicated.
set -uo pipefail
T=/tmp/wt_cpmm_4639; S=/tmp/claude-0/-workspace/55727fa0-a690-442c-a59f-5ed87d136f52/scratchpad
OUT=/workspace/.smoke_int0915/cpmm4639v5_$(date +%H%M%S); mkdir -p $OUT
