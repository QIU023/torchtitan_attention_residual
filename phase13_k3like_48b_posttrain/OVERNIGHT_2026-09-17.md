...
One coverage gap surfaced afterwards and was chased to its end. All ten cells ran on the default context-parallel backend `ulysses`; the other value, `allgather_kv`, fails before step 1 with `torch.compile with fullgraph=True found no compiled frames`, inside the BlockMask sharding that backend alone performs.

Seven candidate causes were measured and excluded: the tree and the backend (the 21-cell matrix has seven `allgather_kv` cells, all passing under plain torchrun), dynamo being disabled (`disable=False`, `is_dynamo_supported=True` measured in the worker), the thread (main, worker and asyncio threads all compile `fullgraph=True` in a three-way comparison), a patched `create_block_mask` (stock, unwrapped, and it compiles), a first compile warming the rest, and anything on the preprocessing path disabling dynamo (`torch_remat`, `spmd_types` and `vllm` carry none).

Reading torch settled it. `_context_parallel/_attention.py` caches its wrapper in a module-level global filled by whichever call reaches it first, so its state is fixed by that first call rather than by the failing one, which is exactly the asymmetry measured: a wrapper built at the failing moment works, the cached one does not. The line stops there deliberately, since the wrapper belongs to torch and pinning which earlier call fills it would change nothing on this side. For the PR body it is one sentence: under the colocated worker `ulysses` runs and `allgather_kv` does not, and every image cell ran on `ulysses`.
