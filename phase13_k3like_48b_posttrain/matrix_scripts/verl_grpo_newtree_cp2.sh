...
export VERL_TORCHTITAN_FLAVOR=${VERL_TORCHTITAN_FLAVOR:-kimi_k3_rl_cp2}
export HF_HOME=/workspace/.hf_home
export FLASHINFER_DISABLE_VERSION_CHECK=1
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
# cp2 cell on the new tree: the rl model under the recipe's CP transforms (kimi_k3_rl_cp2 alias in
# /tmp/wt_k3int_cp), context_parallel_size 2 on two cards, offload off, log-prob micro batch 4.
...
timeout 5400 bash tests/special_e2e/run_ppo_trainer_torchtitan.sh \
  ...
  actor_rollout_ref.actor.torchtitan.context_parallel_size=${CP_SIZE:-2} \
  actor_rollout_ref.ref.torchtitan.context_parallel_size=${CP_SIZE:-2} \
...
rc=$?
grep -aoE "step:[0-9]+ - .*(train/loss|rollout_actor_probs_pearson_corr):[0-9.]+" /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-cp2}.log | tail -4
grep -aiE "Error|Traceback" /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-cp2}.log | grep -v "ERROR:root:initial_load\|deprecat" | tail -5
echo "rc=$rc"
