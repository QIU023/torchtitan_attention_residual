#!/bin/bash
# Part B: the four 8-GPU cells, after part A and the 4-GPU veRL cells.
...
cd /workspace && CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 TITAN=/tmp/wt_int0922b SEED_ROOT=/workspace/.mx4_seeds_0922b VENV=/workspace/venv_bfx9 PYPRE=$S:/workspace/pylib/attn_gym_main CFG=kimi_k3_debugmodel \
  ...
  CELLS="fsdp2_tp2_pp2|8|kimi_k3|kimi_k3_debugmodel|$D2 $TP2 $PP2 --parallelism.num_pp_microbatches 8
...
tp2_cp2_pp2|8|cpmm_probe|kimi_k3_mm_allgather_kv_cp2_pp2_min96|$TP2 --parallelism.no-enable-sequence-parallel --parallelism.num_pp_microbatches 16" $M/mx4b.sh int0922b
echo "MATRIX DONE"
