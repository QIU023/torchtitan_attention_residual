...
# Reward-bearing cell: the synthetic alphabetic-fraction reward gives a random-init policy nonzero
# advantages, so the actor moves and the weight sync is tested by rollout_probs_diff; the control is the
# same cell with KIMI_GRPO_FREEZE_SYNC=1 (the rollout keeps step 1's weights).
...
export TORCHINDUCTOR_CACHE_DIR=${TORCHINDUCTOR_CACHE_DIR:-/workspace/.inductor_verl_reward} TRITON_CACHE_DIR=${TRITON_CACHE_DIR:-/workspace/.triton_verl_reward}
...
actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=${LOGP_MBS:-8} \
  ...
  "$@" > /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-reward}.log 2>&1
rc=$?
grep -aoE "step:[0-9]+ - .*(train/loss|rollout_actor_probs_pearson_corr):[0-9.]+" /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-reward}.log | tail -4
grep -aiE "Error|Traceback" /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-reward}.log | grep -v "ERROR:root:initial_load\|deprecat" | tail -5
echo "rc=$rc"
