#!/bin/bash
# TP/SP + spmd_types declarations on tpsp_spmd_review1 (8e7d4998d on upstream/main 6e2ac3dcd):
# the partial_dtensor dp1 control, then spmd_types on the data / expert / tensor meshes.
set -uo pipefail
...
TITAN=/tmp/wt_tpsprun CFG=kimi_k3_debugmodel BATCH="$B" \
CELLS="dp1|1|$D 1
dp1_spmd|1|$D 1 $S
dp2_spmd|2|$D 2 $S
...
tp4|4|$D 1 $T 4 $S
dp2_tp2|4|$D 2 $T 2 $S
dp2_ep2_tp2|4|$D 2 $E 2 $T 2 $S" $MX tpsp1
echo "TPSP MATRIX DONE"
