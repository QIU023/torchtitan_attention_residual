#!/bin/bash
# GRPO on the 09-16 integration tree (/tmp/wt_int0916_rl = k3_on_4025 rebuild + the local rl flavor) with verl kimi_k3_integration_rebased on upstream main (/tmp/wt_verl_0915).
...
export TORCHINDUCTOR_CACHE_DIR=/workspace/.inductor_verl_int0916 TRITON_CACHE_DIR=/workspace/.triton_verl_int0916
...
timeout 5400 bash tests/special_e2e/run_ppo_trainer_torchtitan.sh \
  ...
  "$@" > /workspace/${VERL_EXP_NAME:-grpo-k3-int0916}.log 2>&1
rc=$?
grep -aoE "step:[0-9]+ - .*(train/loss|rollout_actor_probs_pearson_corr):[0-9.]+" /workspace/${VERL_EXP_NAME:-grpo-k3-int0916}.log | tail -4
grep -aiE "Error|Traceback" /workspace/${VERL_EXP_NAME:-grpo-k3-int0916}.log | grep -v "ERROR:root:initial_load\|deprecat" | tail -5
echo "rc=$rc"
