#!/bin/bash
# pp2 x vp2 cells, compile in-process (two stages per process raced in the compile pool on the same kernel).
set -uo pipefail
...
export SEED_ROOT=/workspace/.mx3_seeds_pp100 MEASURE_STEPS=1 WARM_STEPS=1 TORCHINDUCTOR_COMPILE_THREADS=1
...
run(){ local tree=$1 tag=$2 gpus=$3 cfg=$4 cell=$5; mkdir -p $D/$tag; rm -f $D/$tag/g.*
  TITAN=$tree CUDA_VISIBLE_DEVICES=$gpus TRITON_CACHE_BASE=/tmp/triton_ppnum2_$tag GRAD_DUMP=$D/$tag/g CFG=$cfg BATCH="$B"   CELLS="$cell" bash /workspace/torchtitan_attention_residual/phase13_k3like_48b_posttrain/matrix_scripts/mx3.sh ppnum2_$tag > $D/$tag.mx3.out 2>&1; }
...
wait
for t in vp2c vp2n vp2mut dp1ns; do R=$(ls -dt /workspace/mx3_ppnum2_${t}_0912_* | head -1); echo "$t: $(grep -v '^seed\|^tree\|^DONE' $R/results.txt)"; ls $D/$t; done
echo VP2-DONE
