...
export VERL_TORCHTITAN_FLAVOR=${VERL_TORCHTITAN_FLAVOR:-kimi_k3_rl_lora}
export HF_HOME=/workspace/.hf_home
export FLASHINFER_DISABLE_VERSION_CHECK=1
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
# LoRA cell on the new tree: the rl model under core's LoRA converter (kimi_k3_rl_lora alias in
# /tmp/wt_k3int_cp), merged weight sync (model.lora.merge=True), offload off, log-prob micro batch 4.
...
export TORCHINDUCTOR_CACHE_DIR=/workspace/.inductor_verl_lora TRITON_CACHE_DIR=/workspace/.triton_verl_lora
cd /tmp/wt_verl_cp
...
timeout 5400 bash tests/special_e2e/run_ppo_trainer_torchtitan.sh \
  ...
  actor_rollout_ref.model.lora.merge=${LORA_MERGE:-True} \
actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=${LOGP_MBS:-8} \
  ...
  "$@" > /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-lora}.log 2>&1
rc=$?
grep -aoE "step:[0-9]+ - .*(train/loss|rollout_actor_probs_pearson_corr):[0-9.]+" /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-lora}.log | tail -4
grep -aiE "Error|Traceback" /workspace/${VERL_EXP_NAME:-grpo-k3-newtree-lora}.log | grep -v "ERROR:root:initial_load\|deprecat" | tail -5
echo "rc=$rc"
