...
PLAIN=kimi_k3_debugmodel; CPF=kimi_k3_debugmodel_cp2

streamA() {  # GPUs 0,1
  export CUDA_VISIBLE_DEVICES=0,1 TRITON_CACHE_DIR=/workspace/.triton_mm18_a
  CFG=$PLAIN CELLS="dp1|1|$D 1 $S
fsdp2|2|$D 2 $S
pp2|2|$D 1 $P 2 $MB $S
tp2|2|$D 1 $T 2 $S
ep2_fsdp2|2|$D 2 $E 2 $S" $MX mm18_a2
  CFG=$CPF CELLS="cp2|2|$D 1" $MX mm18_a2cp
}
streamB() {  # GPUs 2-5
  export CUDA_VISIBLE_DEVICES=2,3,4,5 TRITON_CACHE_DIR=/workspace/.triton_mm18_b
  CFG=$PLAIN CELLS="pp4|4|$D 1 $P 4 $MB $S
tp4|4|$D 1 $T 4 $S" $MX mm18_b4
  CFG=$CPF CELLS="cp4|4|$D 1 $C 4" $MX mm18_b4cp
}
streamC() {  # all 8 GPUs, after A and B
  export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 TRITON_CACHE_DIR=/workspace/.triton_mm18_c
  CFG=$PLAIN CELLS="pp8|8|$D 1 $P 8 $MB $S
ep8_fsdp8|8|$D 8 $E 8 $S
fsdp2_tp2_pp2|8|$D 2 $T 2 $P 2 $MB $S
ep2_fsdp2_tp2_pp2|8|$D 2 $E 2 $T 2 $P 2 $MB $S" $MX mm18_c8
...
ep2_fsdp2_tp2_cp2|8|$D 2 $E 2 $T 2 $NOSP" $MX mm18_c8cp
}
