#!/bin/bash
# attn_res_cache_offload on pp_offload_review1 (pp_review3 + the offload commit): pp2 x vp4 and
...
TITAN=/tmp/wt_ppoffrun CFG=kimi_k3_debugmodel_pp_offload BATCH="$B" \
CELLS="pp2_vp4_offload|2|$D 1 $P 2 $L 4 $IL
pp8_vp4_offload|8|$D 1 $P 8 $L 1 $IL" $MX ppoff_pp
echo "PPOFF MATRIX DONE"
