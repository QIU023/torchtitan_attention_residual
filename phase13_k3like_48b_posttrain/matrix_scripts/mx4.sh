...
# Usage: TITAN=<tree> CFG=<seed flavor> BATCH="<flags>" CELLS="<name|nproc|module|config|flags>..." mx4.sh <tag>
...
if [ -s "$S/.built" ] && [ "$(find "$S/seed_ckpt" -type f 2>/dev/null | wc -l)" -gt 0 ]; then
  ...
  ( source ${VENV:-/venv/main}/bin/activate && cd "$TITAN" && PYTHONPATH=${PYPRE:+$PYPRE:}$TITAN timeout 900 torchrun \
...
cell(){ local nm=$1 np=$2 mod=$3 cfg=$4; shift 4
  ...
  for pass in warm measure; do
    ...
    ( source ${VENV:-/venv/main}/bin/activate && cd "$TITAN" && PYTHONPATH=${PYPRE:+$PYPRE:}$TITAN timeout 2400 torchrun \
      --nproc_per_node=$np --master_port=$((30000+RANDOM%20000)) -m torchtitan.train \
      --module $mod --config $cfg --debug.seed $SEED --debug.deterministic \
