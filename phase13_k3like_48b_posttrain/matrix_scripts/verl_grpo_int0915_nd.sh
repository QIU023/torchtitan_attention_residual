#!/bin/bash
# Multi-axis GRPO cells on the 09-15 integration tree (/tmp/wt_int0915_rl, verl /tmp/wt_verl_0915, export -rel) with the synthetic reward as the sync instrument (the actor must move
...
export TORCHINDUCTOR_CACHE_DIR=${TORCHINDUCTOR_CACHE_DIR:-/workspace/.inductor_verl_int0915_nd} TRITON_CACHE_DIR=${TRITON_CACHE_DIR:-/workspace/.triton_verl_int0915_nd}
export ATTN_GYM_CUTE_CACHE_DIR=${ATTN_GYM_CUTE_CACHE_DIR:-/workspace/.cute_verl_int0915_nd}
export TORCHINDUCTOR_COMPILE_THREADS=1
cd /tmp/wt_verl_0915
NUM_GPUS=${NUM_GPUS:-8} FSDP_SIZE=${FSDP_SIZE:-2} EP_SIZE=${EP_SIZE:-1} TP_SIZE=${TP_SIZE:-1} SPMD_BACKEND=${SPMD_BACKEND:-spmd_types} MODEL_ID=kimi-k3-debug-nt MODEL_PATH=${MODEL_PATH:-/root/models/kimi-k3-debug-nt-rel} \
timeout ${CELL_TIMEOUT:-7200} bash tests/special_e2e/run_ppo_trainer_torchtitan.sh \
  ...
  "$@" > /workspace/${VERL_EXP_NAME:-grpo-k3-int0915-nd}.log 2>&1
rc=$?
grep -aoE "step:[0-9]+ - .*rollout_logprobs_diff_mean:[0-9.]+" /workspace/${VERL_EXP_NAME:-grpo-k3-int0915-nd}.log | grep -oE "step:[0-9]+|training/rollout_logprobs_diff_(max|mean):[0-9.e-]+" | paste - - - | tail -4
grep -aiE "Error|Traceback" /workspace/${VERL_EXP_NAME:-grpo-k3-int0915-nd}.log | grep -v "ERROR:root:initial_load\|deprecat\|select_algorithm\|triton_bundler\|lspci\|not available" | tail -3
echo "rc=$rc"
