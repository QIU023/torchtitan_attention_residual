#!/bin/bash
# Multi-axis GRPO cells for an UPSTREAM torchtitan model (default Qwen3-0.6B) through the engine, same knobs as verl_grpo_int0916_nd.sh.
...
unset VERL_TORCHTITAN_FLAVOR  # an upstream model: the flavor is derived from the HF config
...
cd "${VERL_TREE}"
NUM_GPUS=${NUM_GPUS:-8} FSDP_SIZE=${FSDP_SIZE:-2} EP_SIZE=${EP_SIZE:-1} TP_SIZE=${TP_SIZE:-1} SPMD_BACKEND=${SPMD_BACKEND:-spmd_types} MODEL_ID=Qwen/Qwen3-0.6B MODEL_PATH=${MODEL_PATH:-/root/models/Qwen/Qwen3-0.6B} \
timeout ${CELL_TIMEOUT:-7200} bash tests/special_e2e/run_ppo_trainer_torchtitan.sh \
  ...
  "$@" > /workspace/${VERL_EXP_NAME:-grpo-k3-int0916-nd}.log 2>&1
rc=$?
grep -aoE "step:[0-9]+ - .*rollout_logprobs_diff_mean:[0-9.]+" /workspace/${VERL_EXP_NAME:-grpo-k3-int0916-nd}.log | grep -oE "step:[0-9]+|training/rollout_logprobs_diff_(max|mean):[0-9.e-]+" | paste - - - | tail -4
grep -aiE "Error|Traceback" /workspace/${VERL_EXP_NAME:-grpo-k3-int0916-nd}.log | grep -v "ERROR:root:initial_load\|deprecat\|select_algorithm\|triton_bundler\|lspci\|not available" | tail -3
echo "rc=$rc"
