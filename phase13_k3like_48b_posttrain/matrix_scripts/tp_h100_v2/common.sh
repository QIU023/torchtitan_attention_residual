...
NOSP="--parallelism.no-enable-sequence-parallel"; TC="--debug.spmd_typechecking"  # spmd_backend is gone on main after #4419
...
cell() {  # cell <name> <tree> <gpus> <nproc> <seed tag> <steps> <flags...>
  ...
  ( cd $tree && CUDA_VISIBLE_DEVICES=$gpus TORCHINDUCTOR_CACHE_DIR=$OUT/ind_$nm TRITON_CACHE_DIR=$OUT/tri_$nm \
      ...
      $COMMON --training.steps $steps "$@" --dump-folder $d > $OUT/$nm.log 2>&1 )
