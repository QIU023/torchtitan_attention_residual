...
g() { local nm=$1 common=$2 steps=$3 flags=$4; local d=$OUT/$nm; rm -rf $d; mkdir -p $d; cp -al $OUT/seed_c4_256/checkpoint $d/
  ( cd $TT && CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 TORCHINDUCTOR_CACHE_DIR=$J/inductor TRITON_CACHE_DIR=$J/triton \
      torchrun --nproc_per_node=8 --master_port=$((30000+RANDOM%20000)) $common --training.steps $steps $B $D 1 $P8 $flags --dump-folder $d > $OUT/$nm.log 2>&1 )
...
for prec in $PRECS; do
  ...
  g tri_${prec}_warm_n "$NAIVE" 1 "$flags"; g tri_${prec}_warm_c "$COMMON" 1 "$flags"
  g tri_${prec}_n "$NAIVE" $STEPS "$flags"; g tri_${prec}_c "$COMMON" $STEPS "$flags"
  python3 "$(dirname "$0")/repr_compare.py" $OUT/tri_${prec}_n.log $OUT/tri_${prec}_c.log > $OUT/tri_${prec}_compare.txt
  echo "== $prec: $(wc -l < $OUT/tri_${prec}_compare.txt) steps compared"; sed -n '1p;10p;20p;50p;100p' $OUT/tri_${prec}_compare.txt | cut -c1-200
done
echo TRIPLET-DONE
