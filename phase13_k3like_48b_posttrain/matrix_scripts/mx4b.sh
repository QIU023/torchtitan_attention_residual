#!/bin/bash
# Matrix runner reporting steps 1 / 3 / 10; each cell names its own module and config (recipes or flavors of one shape).
# Usage (main after the checkpoint component move: the wrapper module mx4ckpt on PYPRE carries the checkpointer): TITAN=<tree> CFG=<seed flavor> BATCH="<flags>" CELLS="<name|nproc|module|config|flags>..." mx4b.sh <tag>
# Shared seed checkpoint cache, per-cell warm-up pass, seed-load assertion.
set -uo pipefail
TAG=$1
OUT=/workspace/mx4_${TAG}_$(date +%m%d_%H%M%S); mkdir -p "$OUT"
...
SEED_ROOT=${SEED_ROOT:-/workspace/.mx4_seeds}
...
if [ -s "$S/.built" ] && [ "$(find "$S/seed_ckpt" -type f 2>/dev/null | wc -l)" -gt 0 ]; then
  ...
  ( source ${VENV:-/venv/main}/bin/activate && cd "$TITAN" && MX4_BASE_MODULE=${MODULE:-kimi_k3} MX4_BASE_CONFIG=$SEED_CFG PYTHONPATH=${PYPRE:+$PYPRE:}$TITAN timeout 900 torchrun \
    --nproc_per_node=1 --master_port=$((30000+RANDOM%20000)) -m torchtitan.train \
    --module mx4ckpt --config seed --debug.seed $SEED --debug.deterministic --training.steps 1 $BATCH ${SEED_EXTRA:-} \
    --parallelism.data_parallel_shard_degree 1 \
    --dump-folder "$T" > "$OUT/seed.log" 2>&1 ); echo "seed rc=$?" >> $R
...
cell(){ local nm=$1 np=$2 mod=$3 cfg=$4; shift 4
  ...
  for pass in warm measure; do
    ...
    ( source ${VENV:-/venv/main}/bin/activate && cd "$TITAN" && MX4_BASE_MODULE=$mod MX4_BASE_CONFIG=$cfg PYTHONPATH=${PYPRE:+$PYPRE:}$TITAN timeout 2400 torchrun \
      --nproc_per_node=$np --master_port=$((30000+RANDOM%20000)) -m torchtitan.train \
      --module mx4ckpt --config cell --debug.seed $SEED --debug.deterministic \
      --metrics.log_freq 1 --training.steps $steps $BATCH \
      "$@" --dump-folder "$d" > "$OUT/${nm}_$pass.log" 2>&1 )
    rm -rf "$d/checkpoint"
  done
...
while IFS='|' read -r nm np mod cfg flags; do
  [ -z "${nm// }" ] && continue
  cell "$nm" "$np" "$mod" "$cfg" $flags
done <<< "$CELLS"
echo "DONE $OUT" >> $R
