#!/bin/bash
# Part C: part A's 4-GPU tp cells again with ep2 (main's #4794 refuses ep < tp on MoE models; tp2 alone is the tp2_ep2 row), on GPUs 0-3.
...
cd /workspace && CUDA_VISIBLE_DEVICES=0,1,2,3 TITAN=/tmp/wt_int0922b SEED_ROOT=/workspace/.mx4_seeds_0922b VENV=/workspace/venv_bfx9 PYPRE=$S:/workspace/pylib/attn_gym_main CFG=kimi_k3_debugmodel \
  ...
  CELLS="tp2_pp2|4|kimi_k3|kimi_k3_debugmodel|$D1 $TP2 $PP2 --parallelism.num_pp_microbatches 16 $EP2
cp2_tp2_ag|4|cpmm_probe|kimi_k3_mm_allgather_kv_cp2_tp2_min96| $EP2
cp2_tp2_ul|4|cpmm_probe|kimi_k3_mm_ulysses_cp2_tp2_min96| $EP2" $M/mx4b.sh int0922b
echo "MATRIX DONE"
