...
old = "        q_TC, k_TC, v_TC = self.q_proj(x_TD), self.k_proj(x_TD), self.v_proj(x_TD)\n"
assert s.count(old) == 1
new = old + """        _eps = float(__import__("os").environ.get("KDA_PERTURB", "0") or 0)  # LOCAL PROBE HACK (not committed)
        if _eps:
