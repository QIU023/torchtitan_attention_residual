#!/bin/bash
# Image GRPO cells on the 09-22 integration tree (/tmp/wt_int0922b = k3_int_20260922b; export /root/models/kimi-k3-debug-nt-0922b) with verl ${VERL_TREE} (/tmp/wt_verl_new = 515b162c).
...
export PYTHONPATH=${VERL_TREE}:/tmp/wt_int0922b:/workspace/pylib/attn_gym_main
...
timeout ${CELL_TIMEOUT:-7200} bash tests/special_e2e/run_ppo_trainer_torchtitan.sh \
  ...
  "$@" > /workspace/${VERL_EXP_NAME:-grpo-k3-image-int0922b}.log 2>&1
rc=$?
grep -aoE "step:[0-9]+ - .*rollout_logprobs_diff_mean:[0-9.]+" /workspace/${VERL_EXP_NAME:-grpo-k3-image-int0922b}.log | grep -oE "step:[0-9]+|training/rollout_logprobs_diff_(max|mean):[0-9.e-]+" | paste - - - | tail -4
grep -aiE "Error|Traceback" /workspace/${VERL_EXP_NAME:-grpo-k3-image-int0922b}.log | grep -v "ERROR:root:initial_load\|deprecat\|select_algorithm\|triton_bundler\|lspci\|not available" | tail -3
echo "rc=$rc"
