#!/bin/bash
# fp32 masters (training.dtype float32), 13-layer local alias so the 1-GPU cell fits 16 GB; 1 GPU vs 2-GPU PP, 5 steps, two 256-token micro-batches.
MX=/workspace/torchtitan_attention_residual/phase13_k3like_48b_posttrain/matrix_scripts/mx3.sh
export VENV=/workspace/venv_bfx9 SEED_ROOT=/workspace/.mx3_seeds_pprt_fp32l13 SEED_CFG=kimi_k3_debugmodel13 PYPRE=/tmp/attn_gym_up MEASURE_STEPS=5 WARM_STEPS=1
...
TITAN=/tmp/wt_pprt CFG=kimi_k3_debugmodel13 BATCH="$B" CELLS="dp1|1|$D 1 $PD
pp2|2|$D 1 $P 2 --parallelism.num-pp-microbatches 2 $PD" $MX pprt_fp32l13
O=$(ls -td /workspace/mx3_pprt_fp32l13_* | head -1)
python3 - "$O" <<'PY'
import re, sys
o = sys.argv[1]
def series(f):
