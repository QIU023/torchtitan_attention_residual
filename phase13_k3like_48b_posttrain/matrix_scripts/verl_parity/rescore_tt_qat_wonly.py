...
from torchtitan.models.kimi_k3 import model_registry as _mr
cfg = _mr("rl", converters=[MXFP4QATConverter.Config(quantize_act=False)]).model; model = cfg.build()
hf = {}
...
torch.save(out, f"{D}/rescore_tt_qat_wonly.pt"); print("saved", len(out), "sequences", flush=True)
