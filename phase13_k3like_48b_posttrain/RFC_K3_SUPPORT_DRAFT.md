...
- AttnRes primitive + a Kimi-Linear port (KDA via `fla-core`, MLA, MoE) with FSDP2 / TP / EP parallelization — [implementation](https://github.com/QIU023/torchtitan/tree/f7bb7cd0f/torchtitan/experiments/kimi_k3). GPU-verified on the current fork (2026-07-19): FSDP2 and PP with full numerics runs; TP and EP migrated to trunk's module-internal MoE mechanism (`set_moe_sharding_config` + `Module.parallelize`, same pattern as `deepseek_v3`) and train-smoke verified — the TP/EP-vs-FSDP loss-parity matrix is queued next.
