...
- AttnRes primitive + a Kimi-Linear port (KDA via `fla-core`, MLA, MoE) with FSDP2 / TP / EP parallelization — [implementation](https://github.com/QIU023/torchtitan/tree/469577cdf/torchtitan/experiments/kimi_k3).
- PP support via a cross-stage adapter kept **private to the model folder's `parallelize`** — no core changes, per earlier feedback on the generic-mechanism proposal: [adapter](https://github.com/QIU023/torchtitan/blob/469577cdf/torchtitan/experiments/kimi_k3/pipeline_adapter.py), [design notes + pressure-test launchers](https://github.com/QIU023/torchtitan_attention_residual/tree/main/phase3_attnres_pp_integration).
...
- Vision-path precedent (de-risking the vision-native scope): a SigLIP-based multimodal scaffold (vision-token splice into the AttnRes LM stream) is part of the experiment — [multimodal model](https://github.com/QIU023/torchtitan/blob/469577cdf/torchtitan/experiments/kimi_k3/kimi_linear/multimodal_model.py), [CPU test](https://github.com/QIU023/torchtitan/blob/469577cdf/torchtitan/experiments/kimi_k3/tests/test_kimi_multimodal_model.py) — and the full LLaVA-1.5-style pipeline was exercised end-to-end on the 447M carrier: alignment pretraining on LLaVA-Pretrain-558K (loss 2.23 -> 1.85), visual-instruction SFT on LLaVA-Instruct-150K, and a stable multimodal GRPO loop, with the cross-stage PP adapter extended to variable-length vision-text sequences under pipeline-parallel static-shape constraints — [phase-5 VLM training](https://github.com/QIU023/torchtitan_attention_residual/tree/main/phase5_vlm_multimodal_sft), [phase-11 post-training](https://github.com/QIU023/torchtitan_attention_residual/tree/main/phase11_rlhf_grpo_infra). Architecture-level evidence only; K3's actual vision design lands with the report.
- CPU unit tests for the primitive, model, and pipeline adapter — [tests](https://github.com/QIU023/torchtitan/tree/469577cdf/torchtitan/experiments/kimi_k3/tests).
