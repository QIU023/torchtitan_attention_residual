...
- AttnRes primitive + a Kimi-Linear port (KDA via `fla-core`, MLA, MoE) with FSDP2 / TP / EP parallelization — [implementation](https://github.com/QIU023/torchtitan/tree/b27d06abb/torchtitan/experiments/kimi_k3). GPU-verified on the current fork (2026-07-19): FSDP2 and PP with full numerics runs; TP and EP migrated to trunk's module-internal MoE mechanism (`set_moe_sharding_config` + `Module.parallelize`, same pattern as `deepseek_v3`) and parity-verified against FSDP from a shared seed checkpoint (fp32 deterministic: step-1 loss exact for both TP and EP, early-window |Δloss| ≤ 5e-5).
- PP support via a cross-stage adapter kept **private to the model folder's `parallelize`** — no core changes, per earlier feedback on the generic-mechanism proposal. Non-Interleaved1F1B schedules fall back to the plain pipeline path (correct, without the cache saving): [adapter](https://github.com/QIU023/torchtitan/blob/b27d06abb/torchtitan/experiments/kimi_k3/pipeline_adapter.py), [design notes + pressure-test launchers](https://github.com/QIU023/torchtitan_attention_residual/tree/main/phase3_attnres_pp_integration).
...
- Multimodal (vision-native) precedent: SigLIP-splice scaffold in the experiment ([model](https://github.com/QIU023/torchtitan/blob/b27d06abb/torchtitan/experiments/kimi_k3/multimodal_model.py), [CPU test](https://github.com/QIU023/torchtitan/blob/b27d06abb/torchtitan/experiments/kimi_k3/tests/test_kimi_multimodal_model.py)); LLaVA-1.5-style pretraining + SFT + GRPO exercised end-to-end on the 447M carrier — [phase-5 VLM training](https://github.com/QIU023/torchtitan_attention_residual/tree/main/phase5_vlm_multimodal_sft), [phase-11 post-training](https://github.com/QIU023/torchtitan_attention_residual/tree/main/phase11_rlhf_grpo_infra).
- Unit tests for the primitive, model, state-dict adapter, and pipeline adapter — [tests](https://github.com/QIU023/torchtitan/tree/b27d06abb/torchtitan/experiments/kimi_k3/tests); full suite green on GPU (52 passed / 0 skipped incl. the fla-triton, FlexAttention-backward, and fp8 hardware paths), 2026-07-19.
