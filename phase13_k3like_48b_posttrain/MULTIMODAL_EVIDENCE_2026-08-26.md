...
**待补:seq 1024 的 dp1/cp2/cp4/cp8 表**(`mx3_mm_dyncp8_*`)。cp8 需要
`Q_LEN % (cp * 128) == 0`,所以 seq 必须抬到 1024,那是**另一张表、自带 dp1**,
不能作为本表的第四行。

它要回答的是:本表 cp2 -> cp4 偏差涨了 13 倍(2.62e-4 -> 3.52e-3),而文本侧 CP 从
cp4 到 cp8 是**饱和**的(`EP_TEXT_EVIDENCE.md` 第一节记过同形现象)。两个度数分不出
趋势和单点 —— **度数要跑到头再下结论**,这是 EP 表 `ep8` 推翻预判那次留下的纪律。
