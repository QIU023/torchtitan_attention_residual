...
### 7.4 bubble 的数值中性(补验,2026-08-27)

7.1 只测了时间。回头核 loss 时先撞上一个假信号:三臂里 off 的 s1 与另外两臂差
1e-3。判别依据有二:**被守卫忽略、实际跑基线的 prefetch 臂等于 bubble 臂而不等于
off 臂**(惰性开关不可能通过机制改数值);同源码连跑两次 off **逐位一致**。差异
跟随编译缓存状态而非开关 —— inductor autotune 混杂第三次应验(前两次见
gate-inductor-cache-confound)。

缓存收敛后受控对比(同一缓存会话,紧挨着跑):

| | off | bubble | 判定 |
|---|---|---|---|
| step 1 | 12.51982 | **12.51982** | **逐位相同**:cut 是 detach,前向值不变 |
| step 2 | 9.51229 | 9.47524 | 3.9e-3:反向重放改变塔梯度累加顺序,bf16 非逐位 |

结论:bubble **前向逐位中性、反向非逐位** —— 与 delta transport 同一接受类别
("送到的块相同,求和顺序不同")。调度确认同行照打:8/8 计划槽,0 兜底。

方法学教训:**时间 A/B 与数值 A/B 是两个实验**。7.1 的三臂各自换源码(sed 翻默认
值),代码 hash 变化触发重编译,数值经过 autotune 走样 —— 那批 loss 不可用于
数值判定,幸而当时只取了时间。数值判定必须在缓存收敛后同会话紧挨着跑。
