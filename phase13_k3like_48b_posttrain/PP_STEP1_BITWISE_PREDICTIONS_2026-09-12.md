...
- P7a: refuted. Activation checkpointing off in both (flex pinned, matched accumulation): pp2 vs dp1 is 20 / 750 bitwise, the same numbers as every other pipeline cell. The residual is invariant to the split (pp2, pp2 x vp2), the flag (on, off), flex autotune (on, off) and activation checkpointing (on, off); it starts inside layer 23's attention backward on the last stage, before any gradient crosses a stage boundary. Its source is how a pipeline process runs one stage's backward, not the transport. Not chased further in this campaign.
