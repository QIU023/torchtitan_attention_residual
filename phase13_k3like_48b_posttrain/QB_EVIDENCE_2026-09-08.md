...
| hook registration | the flavor installs the solver as the model spec's `post_optimizer_build_fn`, the slot core's sign-rule registration occupies; the trainer calls one such function, so the sign rule is NOT registered under the flavor. The module's earlier comments said core's hook still ran first; corrected in `0e52d7b46`, which also resets the MoE's `tokens_per_expert_E` in core's place | text was wrong, math unaffected |
