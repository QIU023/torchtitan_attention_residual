...
## Gate 输出的一个显示 bug(2026-08-18 发现,待 gate 跑完再修)

`run_maxdeg.sh` 的收集器**没有按行去重**,所以每个 rank 打的同一步都进了表。表现是 loss
看起来一步不动:

    ep8_fsdp8        7.71120 7.71120 7.71120 ...
    tp4              7.70643 7.70643 7.70643 ...
    cp4              7.70812 7.70812 7.70812 ...

而同一张表里 pp4/pp8 正常下降 —— 因为 PP 只有最后一个 stage 打 loss,天然不重复。原始日志
是好的:cp4 实际是 7.70812 → 7.69513 → 7.64384。

**范围要说准:只是显示,判定是对的。** 同一个文件里的 `rows()`(第 27 行)用
`sort -u -n -k1,1 | grep -c .` 按步号去重后再计数,pass/fail 走的是它 —— 所以这三格是**真的
跑满 10 步并通过了**,accounting 没问题。出问题的只有第 48–50 行那句打印:它 grep 出全部
loss 行就直接 `head -3`,4 个 rank 于是把第 1 步打了三遍。

**为什么值得修而不是忍**:一个 8 卡最高并行度的格子显示成"loss 完全不动",最自然的读法是
"这格坏了",而它其实是好的;反过来,如果哪天它**真的**不动了,这张表也说不出区别。gate 的
全部价值就是能读。

修法就在同一个文件里 —— 把 `rows()` 已经在用的 `sort -u -n -k1,1` 加到打印那条管线的
`head -3` 之前。不用去抄 `collect13.sh`。
