...
`collect13.sh` 早就处理了这件事(它的文件头里写着"dedup on the whole line, not the step
number",还记着一次 dedup-then-filter 把所有行删光的教训),`run_maxdeg.sh` 没跟上。

**为什么值得修而不是忍**:一个 8 卡最高并行度的格子显示成"loss 完全不动",最自然的读法是
"这格坏了",而它其实是好的;反过来,如果哪天它**真的**不动了,这张表也说不出区别。gate 的
全部价值就是能读。

修法是把 `collect13.sh` 的 losses() 抄过去,不是重写一个。
