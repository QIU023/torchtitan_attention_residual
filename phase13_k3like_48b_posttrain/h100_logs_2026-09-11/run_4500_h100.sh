...
source /venv/main/bin/activate; export PYTHONPATH=/workspace/attn_gym_up:/workspace/wt_4500
...
run cp1 0 1 --module kimi_k3 --config kimi_k3_debugmodel --parallelism.data_parallel_shard_degree 1 --parallelism.spmd_backend spmd_types
run cp2ag 0,1 2 --module torchtitan_recipes.tests.b200 --config kimi_k3_debugmodel_mm_cp2 --parallelism.data_parallel_shard_degree 1
run cp2u 0,1 2 --module torchtitan_recipes.tests.b200 --config kimi_k3_debugmodel_mm_ulysses_cp2 --parallelism.data_parallel_shard_degree 1
python3 - <<"PY"
import re
def series(f):
    L,G={},{}
