#!/bin/bash
# The 18-cell TEXT-arm matrix (c4_test, run-worktree text aliases) on the integration tree 6dbc6805b; derived from the multimodal parallelism matrix (MATRIX_18_CORRECTED_2026-08-09.md's cells) on the
...
export SEED_ROOT=/workspace/.mx3_seeds_text18b SEED_CFG=kimi_k3_debugmodel_text PYPRE=/tmp/attn_gym_up
export TITAN=/tmp/wt_text18
...
PLAIN=kimi_k3_debugmodel_text; CPF=kimi_k3_debugmodel_text_cp2

streamA() {  # GPUs 0,1
  export CUDA_VISIBLE_DEVICES=0,1 TRITON_CACHE_DIR=/workspace/.triton_text18_a
  CFG=$PLAIN CELLS="dp1|1|$D 1 $S
fsdp2|2|$D 2 $S
pp2|2|$D 1 $P 2 $MB $S
tp2|2|$D 1 $T 2 $S
ep2_fsdp2|2|$D 2 $E 2 $S" $MX text18_a2
  CFG=$CPF CELLS="cp2|2|$D 1" $MX text18_a2cp
}
streamB() {  # GPUs 2-5
  export CUDA_VISIBLE_DEVICES=2,3,4,5 TRITON_CACHE_DIR=/workspace/.triton_text18_b
  CFG=$PLAIN CELLS="pp4|4|$D 1 $P 4 $MB $S
tp4|4|$D 1 $T 4 $S" $MX text18_b4
  CFG=$CPF CELLS="cp4|4|$D 1 $C 4" $MX text18_b4cp
}
...
ep2_fsdp2_tp2_pp2|8|$D 2 $E 2 $T 2 $P 2 $MB $S" $MX text18_c8
...
ep2_fsdp2_tp2_cp2|8|$D 2 $E 2 $T 2 $NOSP" $MX text18_c8cp
}
streamA & streamB & wait
streamC
echo "MM18 MATRIX DONE"
echo "TEXT18 DONE"
