...
OUT=/workspace/ppbal_pressure5_$(date +%m%d_%H%M%S); mkdir -p $OUT; R=$OUT/results.txt; : > $R
...
arm() { local name=$1 cfg=$2 tok=$3; shift 3
  ...
  ( source /venv/main/bin/activate && cd $TITAN && CUDA_VISIBLE_DEVICES=0,1,2,3 PYTHONPATH=$TITAN:/workspace/torchtitan_attention_residual/phase13_k3like_48b_posttrain/matrix_scripts/mempeak_site timeout 2400 torchrun --nproc_per_node=4 --master_port=$((30000+RANDOM%20000)) -m torchtitan.train $COMMON --config $cfg --training.num-tokens-per-microbatch-per-dp-rank $tok --training.num-tokens-per-train-step $((tok*8)) --dump-folder $d "$@" > $d/run.log 2>&1 ); local rc=$?
...
# the two decisive arms again, with per-rank allocator peaks printed at exit
TOK=${TOK:-4096}
arm base$TOK kimi_k3_debugmodel_32l $TOK
K3_PPBAL_SRC=0,1 K3_PPBAL_DST=3 K3_PPBAL_POOL_GIB=5 arm ppbal${TOK}_src01 kimi_k3_debugmodel_32l_ppbal $TOK
for a in base$TOK ppbal${TOK}_src01; do echo "--- $a" >> $R; grep -ah "MEMPEAK\|PPBAL.*stats" $OUT/$a/run.log | sort | cut -c1-150 >> $R; done
echo "DONE $OUT" >> $R
