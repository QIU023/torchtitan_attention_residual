...
| `tp_review1` (4 commits on `k3_ep`) | TP rewritten for main's `InnerKDA`: head-parallel MLA and KDA behind a `local_map`, SP overlay, grad-norm mesh grouping, latent-MoE norm under SP | dp1 bitwise with main; tp2 12.55057 / 7.52677 / 3.00361; tp2+SP see ledger |
...
| verl `kimi_k3_integration` = `d778b810` | pipeline parallelism in the torchtitan engine: one pipeline microbatch per stage, verl micro-batches fed in chunks with zero-loss padding, verl's loss reaching the last stage through a bridge on the schedule | pp2 cell, see ledger |
...
| pp2 (rl_vit1, engine PP) | see ledger | |
