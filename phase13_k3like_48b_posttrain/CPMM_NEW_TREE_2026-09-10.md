...
## A hang the matrix found: pipeline x context parallel (2026-09-10, 05:46)

Every C-stream cell combining PP and CP (`fsdp2_pp2_cp2`, `ep2_fsdp2_pp2_cp2`, `tp2_pp2_cp2`) stopped right after "Applied SelectiveAC" and sat at 0% utilisation until the 40-minute timeout. Cause: `_build_cp_subgroups` is a world-wide collective (`dist.new_group` per layout and an `all_gather_object` of the CP rank lists), and `parallelize_kimi_k3` called it only on ranks whose model part holds the vision encoder; under PP the later stages hold none and skipped it, so the tower-holding ranks waited in `new_group` for the rest of the world. The two-rank probe and the cp2/cp4 cells could not see this: without PP every rank holds the tower. Fix `da7f9e348` (branch) and the amended `k3_cp_mm_v2` head `a063a3d0e`: every rank builds the layouts when CP is on, a stage without the tower simply never uses the dict. The five CP cells of the C stream are being rerun on the fixed snapshot; the plain C cells (pp8, ep8_fsdp8, fsdp2_tp2_pp2, ep2_fsdp2_tp2_pp2) had already passed.

GPU cells on the head (own seed, after the sub-CP group fix): dp1 12.54425 / 6.71085, cp2 Ulysses 12.56483 / 6.96542, cp2 all-gather 12.56483 / 6.92415 (steps 1 and 3; the CP cells +0.16% at step 1, the tower's bf16 class). PR 4380's branch `k3_cp_mm` now points at this head (force-pushed 2026-09-10, replacing the old-tree head a5339256e).
