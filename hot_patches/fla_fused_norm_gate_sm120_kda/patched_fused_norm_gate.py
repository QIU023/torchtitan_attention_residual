...
@triton.autotune(
    ...
    # PATCHED-2026-05-17 (PR13 hot_patches/): remove NB from key to prevent
    # autotuner re-runs on each new T range; the kernel body doesn't use NB.
    # Matches PR #796 fix in layernorm.py.
...
@triton.autotune(
    configs=[triton.Config({"BT": BT}, num_warps=num_warps) for BT in [16, 32, 64] for num_warps in [4, 8, 16]],
    # PATCHED-2026-05-17 (PR13 hot_patches/): remove NB from key — same as fwd.
...
def layer_norm_gated_bwd(
    ...
    if D > BD:
        raise RuntimeError("This layer norm doesn't support feature dim >= 64KB.")
    # PATCHED-2026-05-17 (PR13 hot_patches/): mirror PR #796 layernorm.py fix.
    # On high-SM GPUs (B200 188 SMs, RTX 5090 170 SMs) with small T, the
    # original BS = ceil(T / min(SM, T)) can produce BS < max autotuned BT (=64),
    # which makes adjacent programs' make_block_ptr write regions overlap on dx
    # and corrupts GPU memory (CUDA: illegal memory access / device-side assert).
    # Cap NS so BS >= max_BT.
