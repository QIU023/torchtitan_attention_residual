...
cd "$WS"
LOG="$WS/phase3/run_overnight_chain.log"
> "$LOG"
exec >>"$LOG" 2>&1
...
disk_guard() {
    ...
    if [[ "${free_gib:-0}" -lt 50 ]]; then
        ...
        rm -rf "$WS"/phase3/runs/pressure_test_2026051[01]-* 2>/dev/null
        find "$WS"/phase3/runs -name "checkpoint" -type d -exec rm -rf {} + 2>/dev/null
        free_gib=$(df -BG /workspace | tail -1 | awk '{print $4}' | tr -d 'G')
...
commit_push() {
    ...
    git add \
        phase3/PRESSURE_TEST_REPORT_2026-05-12.md \
        phase3/run_kimi48b_*.sh \
        phase3/run_overnight_pp_naive_then_grpo.sh \
        phase3/run_l24_adapter_sweep.sh \
        phase3/run_l32n8_widen_smoke.sh \
        phase3/run_pp_pressure_test.sh \
        phase4/run_kimi48b_downscale_sweep.sh \
...
ADAPTER_LOG="$WS/phase3/run_kimi48b_d1280_e16_L32N8_pp8vp4_adapter.log"
...
echo "==============================================================="
OUT1="$WS/phase3/runs/kimi48b_d1280_e32_L24N8_pp8vp3_naive_$(date +%Y%m%d-%H%M%S)"
rm -rf "$OUT1"
(cd torchtitan && \
...
echo "==============================================================="
OUT2="$WS/phase3/runs/kimi48b_d1280_e16_L32N8_pp8vp4_naive_$(date +%Y%m%d-%H%M%S)"
rm -rf "$OUT2"
(cd torchtitan && \
 env PYTORCH_ALLOC_CONF="expandable_segments:True" \
     torchrun \
...
echo "==============================================================="
python3 "$WS/phase3/gen_pp_report.py" 2>&1 || echo "(no gen_pp_report.py, skip auto-report)"
cd "$WS"
# Stage only the files we expect — never auto-add unknown stuff
git add phase3/PRESSURE_TEST_REPORT_2026-05-12.md \
        phase3/run_kimi48b_d1280_e32_L24N8_pp8vp3_adapter.sh \
        phase3/run_kimi48b_d1280_e32_L32N8_pp8vp4_adapter.sh \
        phase3/run_kimi48b_d1280_e16_L32N8_pp8vp4_adapter.sh \
        phase3/run_overnight_pp_naive_then_grpo.sh \
        phase4/run_kimi48b_downscale_sweep.sh \
        torchtitan 2>&1 || true
...
mkdir -p "$WS/phase11/hf"
if [[ ! -e "$WS/phase11/hf/vlm_sft_3ep" ]]; then
    ln -s /root/torchtitan_attention_residual/phase11/hf/vlm_sft_3ep "$WS/phase11/hf/vlm_sft_3ep"
fi
# DCP path from yesterday's run (may not exist in /workspace — try both)
S3_DCP_ROOT=/root/torchtitan_attention_residual/phase5/runs/vlm_447m_sft_3ep/checkpoint
LAST=$(ls "$S3_DCP_ROOT" 2>/dev/null | tail -1 || echo "")
S3_DCP="$S3_DCP_ROOT/$LAST"
S3_HF="$WS/phase11/hf/vlm_sft_3ep"
S3_OUT="$WS/phase11/rlhf/outputs/grpo_overnight_$(date +%Y%m%d-%H%M%S)"
mkdir -p "$S3_OUT"
