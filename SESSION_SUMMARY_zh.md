...
对应 `phase6_upstream_pr_prep/PR_DRAFT.md` 中所列的"upstream torchtitan AttnRes
merge readiness"目标。
...
| 路径 | 大小 | Loss | 用途 |
|---|---|---|---|
| `phase4_kimi_attnres_lm_pretrain/runs/kimi_436m_block_attn_res_fsdp/checkpoint/step-8000` | 14 GB | C4 val 3.23 | Phase 4 LM-only 起点（未训过多模态）|
| `phase5_vlm_multimodal_sft/runs/arm1prime_fsdp_seed42_from_p4_8k/checkpoint/step-4000` | 14 GB | 3.03 | 第一阶段 caption-quality 终点（GBS=32, 4000 步）|
| `phase5_vlm_multimodal_sft/runs/v8_pretrain_resilient_from_v7_step800/checkpoint/step-10000` | 14 GB | **2.07** | **Phase 6 主成果**：v8 final，含 mm_projector，可直接用于 inference |
| `phase5_vlm_multimodal_sft/runs/v9_continue_from_v8_step10000/checkpoint/step-{3400,3600,3800}` | 42 GB | 2.10 (live) | v9 继续 pretrain，正在跑（5000 步目标）|
...
| 项 | 状态 | 关键 commit | 关键结果 |
|---|---|---|---|
| **A1** real-multimodal alignment (FSDP=4 PP=1 vs FSDP=1 PP=4 V=2 + cache adapter) | ✅ done | `b270b1d`, `2d69453` | median \|Δ\|=0.024 nats, max 0.252 (warmup transient)。`phase6_upstream_pr_prep/alignment_arm2_real_mm_v2.png` |
...
| **A2 partial** PP=4 V=4-per-rank (16 virt stages) | ✅ done | n/a (tb 在 `phase5_vlm_multimodal_sft/runs/a2_pp4_v4perrank_adapter_gbs16_seed42/`) | step-500 loss 3.48；V=4-per-rank 调度 loss-invariant |
...
| **A6 partial** FSDP=2 PP=2 V=2 + adapter | ✅ done | (tb in `phase5_vlm_multimodal_sft/runs/a6_fsdp2_pp2_gbs12_seed42/`) | step-500 \|Δ\|=0.006 nats vs FSDP=1 PP=4 baseline |
...
| **B2** image-text interleave dataset | ✅ done | `2f83d52`, `d1a2177` | `phase5_vlm_multimodal_sft/multimodal_dataset_interleave.py` 包装器 + 7 单元测试；`--mm.layout {prefix,interior,random}` flag 集成进 train_mm |
| **B3** vision tower FSDP-shard | deferred | n/a | 当前 SigLIP-Base 92M 太小不必，有 >4GB/rank vision encoder 时再做 |
| **B4** tokenizer-aware sentinel registry | ✅ done | `bffbbdf`, `d1a2fcf` | `phase5_vlm_multimodal_sft/sentinel_registry.py` (Llama-3, Llama-3.1, Kimi 三个条目) + 9 单元测试 + train_mm 集成 |
| **B5 partial** caption generation script | ✅ done | `be78a37`, `282a9fe` | `phase5_vlm_multimodal_sft/generate_caption.py`：单进程 inference，从 mm_projector ckpt entry 加载已训练 projector，生成 "the ultimate swaddle for baby" 验证端到端工作 |

### C 类 (polish)
| 项 | 状态 | 关键 commit | 关键结果 |
|---|---|---|---|
| **C1** cache adapter ablation | ✅ done | `435f89f`, `e366732` | `phase6_upstream_pr_prep/cache_adapter_ablation.md`：closed-form bytes-saved 公式 ≈ (N+1)/2，4× 比率在我们 scale；500-step naive vs adapter loss alignment max \|Δ\|=0.092 nats PASS |
...
| **C4** perf regression CI | ✅ done | `6254286` | `phase6_upstream_pr_prep/perf_regression_check.py`：5 个 baseline + 5% 容差，对 v8 train.log 已验证 PASS |
...
d1a2177 phase5_vlm_multimodal_sft/train_mm: --mm.layout flag wires interleave dataset
4f9828a phase6: v8 final summary — loss 2.07 final / 1.90 best, B5 verified
282a9fe phase5_vlm_multimodal_sft/generate_caption: fix DCP key prefix + load trained projector
86da5b9 phase6_upstream_pr_prep/README: 2026-05-01 status board for late-day burst
2f83d52 phase6_upstream_pr_prep/B2: image-text interleave dataset wrapper + 7 unit tests
49b3351 phase6_upstream_pr_prep/A5: redo orchestrator with strict step-line grep filter
be78a37 phase6_upstream_pr_prep/B5 partial: caption generation smoke script
6254286 phase6_upstream_pr_prep/C4: throughput regression check script
...
eac6851 phase6_upstream_pr_prep/README: status board for 2026-04-30 burst (A1/A1.1, B1, B4, C1, C2 partial)
655a0fd phase5_vlm_multimodal_sft/compare_pp_vs_fsdp: --out-plot option + emit phase6 A1 figure
0aa8453 phase6_upstream_pr_prep/C2: mixed-dtype scatter test
ebbfe3c phase6: PR draft for upstream torchtitan AttnRes merge
d1a2fcf phase6_upstream_pr_prep/B4 wire-up: train_mm uses sentinel_registry for image-token id
435f89f phase6_upstream_pr_prep/C1: cache adapter ablation report (analytic + empirical)
bffbbdf phase6_upstream_pr_prep/B4: tokenizer-aware image-sentinel registry
e0e4b1d phase6_upstream_pr_prep/B1: variable image count per row + 6 unit tests
b270b1d phase6_upstream_pr_prep/A1: alignment passes after projector grad-sync fix
2d69453 phase5_vlm_multimodal_sft/train_mm: FSDP2-wrap projector on dp/batch mesh — fix A1 alignment
9ea3c8a phase6_upstream_pr_prep/A1: alignment FAILs at median 0.20 / max 0.46 nats
db816ef phase6: alignment v2 — drop GBS=32, use documented GBS=12
59cdb48 phase6_upstream_pr_prep/orchestrator: kill no-seed Arm 1, fold caption story into seeded Arm 1'
8bb6067 phase6: A1 alignment orchestrator + kernel/8-GPU scope updates
caf1ff3 phase6_upstream_pr_prep/readme: add A6/B5/C4, defer B3, parameterize for shape change
...
| 文件 | 用途 |
|---|---|
| `phase6_upstream_pr_prep/README.md` | Phase 6 主文档：scope, A/B/C 任务表, status board (multi-day) |
| `phase6_upstream_pr_prep/PR_DRAFT.md` | upstream torchtitan PR description 草稿（verified configs + resilience features + test coverage）|
| `phase6_upstream_pr_prep/cache_adapter_ablation.md` | C1 ablation report（analytic + empirical）|
| `phase6_upstream_pr_prep/overnight_pretrain_summary.md` | 第一夜 pretrain 总结 (loss 2.79) |
| `phase6_upstream_pr_prep/v8_final_summary.md` | v8 13.5h 跑完总结 (loss 1.90 / 2.07) |
| `phase6_upstream_pr_prep/alignment_report_arm2_real_mm_v1.txt` + `v1.csv` | A1 v1 (FAIL) 报告 |
| `phase6_upstream_pr_prep/alignment_report_arm2_real_mm_v2.txt` + `v2.csv` + `v2.png` | A1 v2 (PASS) 报告 + 图 |
| `phase6_upstream_pr_prep/c1_adapter_vs_naive_report.txt` + `.csv` + `.png` | C1 naive vs adapter 对比 |
| `phase6_upstream_pr_prep/run_a1_alignment_v2.sh` | A1 alignment orchestrator |
| `phase6_upstream_pr_prep/run_a5_redo.sh` | A5 mid-save resume orchestrator |
| `phase6_upstream_pr_prep/run_v8_crash_resilient_pretrain.sh` | v8 crash-resilient orchestrator |
| `phase6_upstream_pr_prep/run_v9_continue_pretrain.sh` | v9 continuation orchestrator |
| `phase6_upstream_pr_prep/perf_regression_check.py` | C4 throughput baseline check |
| `phase5_vlm_multimodal_sft/sentinel_registry.py` + tests | B4 tokenizer-aware sentinel |
| `phase5_vlm_multimodal_sft/multimodal_dataset_interleave.py` + tests | B2 interleave dataset |
| `phase5_vlm_multimodal_sft/generate_caption.py` | B5 generation smoke |
| `phase6_upstream_pr_prep/SESSION_SUMMARY_zh.md` | **本文档** |
